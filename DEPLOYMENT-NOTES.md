...
3. **CORS bloqué sur `news.aeromorning.com`** (`Access-Control-Allow-Origin` absent) — le middleware `HandleCors` est bien enregistré dans `Kernel.php`, donc pas un bug de code : un `bootstrap/cache/config.php` périmé (laissé par un déploiement manuel antérieur à ce pipeline) shadowait le `.env` réel en prod. Corrigé en ajoutant `php artisan config:clear` après chaque déploiement (commit `f6ba5834`).
