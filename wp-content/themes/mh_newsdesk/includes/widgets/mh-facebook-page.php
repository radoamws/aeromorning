...
class mh_newsdesk_facebook_page extends WP_Widget {
	function __construct() {
		parent::__construct(
			'mh_newsdesk_facebook_page', esc_html_x('MH Facebook Page', 'widget name', 'mh-newsdesk'),
			array('classname' => 'mh_newsdesk_facebook_page', 'description' => esc_html__('Widget to display your Facebook page on your website.', 'mh-newsdesk'))
		);
	}
    function widget($args, $instance) {
    	$defaults = array('title' => __('Follow on Facebook', 'mh-newsdesk'), 'fb_url' => 'https://www.facebook.com/MHthemes', 'width' => 373, 'height' => 500, 'cover' => 0, 'faces' => 1, 'posts' => 0);
    ...
    function form($instance) {
		...
		<p>
        	<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'mh-newsdesk'); ?></label>
			<input class="widefat" type="text" value="<?php echo esc_attr($instance['title']); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" id="<?php echo esc_attr($this->get_field_id('title')); ?>" />
        </p>
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('fb_url')); ?>"><?php esc_html_e('Facebook Page URL:', 'mh-newsdesk'); ?></label>
       	...
       	<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php esc_html_e('Width (min. 280px):', 'mh-newsdesk'); ?></label>
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('height')); ?>"><?php esc_html_e('Height (min. 130px):', 'mh-newsdesk'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('cover')); ?>"><?php esc_html_e('Hide cover photo', 'mh-newsdesk'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('faces')); ?>"><?php esc_html_e('Show profile photos', 'mh-newsdesk'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('posts')); ?>"><?php esc_html_e('Show posts from FB page', 'mh-newsdesk'); ?></label>
		</p><?php
    }
}
