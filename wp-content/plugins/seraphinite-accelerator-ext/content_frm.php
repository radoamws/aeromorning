...
function GetYouTubeVideoThumbUrl( &$ctxProcess, $id, $args = null )
{
	if( $id == 'videoseries' || $id == 'live_stream' )
	{
		...
		else if( @preg_match( '@\\Wytcfg\\s*.\\s*set\\s*\\(\\s*{@', $data, $m, PREG_OFFSET_CAPTURE ) )
		{
			...
			$m = Gen::GetArrField( $m, array( 'previewPlayabilityStatus', 'errorScreen', 'playerErrorMessageRenderer' ), array() );

			$mTest = Gen::GetArrField( $m, array( 'proceedButton', 'buttonRenderer', 'navigationEndpoint', 'urlEndpoint', 'url' ), '' );
			if( $mTest )
				return( GetVideoThumbUrlFromUrl( $ctxProcess, $mTest ) );

			$mTest = Gen::GetArrField( $m, array( 'reason', 'runs', 0, 'text' ), '' );
			if( $mTest && preg_match( '@<a\\s[^>]*href=[\'"]([^\'"]+)@', $mTest, $m2 ) )
				return( GetVideoThumbUrlFromUrl( $ctxProcess, $m2[ 1 ] ) );
		}
	}
...
function ApplyYouTubeVideoPlaceholder( $item, &$src, $lazyVideoCurId, $urlThumb, $sz = null )
{
	$data = '<!DOCTYPE html>' . Ui::Tag( 'html', Ui::Tag( 'body',
			Ui::Tag( 'a', null, array( 'onclick' => 'window.parent.postMessage(\'seraph-accel-lzl-v:' . $lazyVideoCurId . '\',\'*\');', 'style' => array( 'cursor' => 'pointer', 'position' => 'absolute', 'width' => '100%', 'height' => '100%', 'background' => 'center / cover no-repeat url(' . $urlThumb . ')' ) ) ) .
...
function ApplyVimeoVideoPlaceholder( $item, &$src, $lazyVideoCurId, $urlThumb, $sz = null )
{
	...
	$data = '<!DOCTYPE html>' . Ui::Tag( 'html', Ui::Tag( 'body',
			Ui::Tag( 'a', null, array( 'onclick' => 'window.parent.postMessage(\'seraph-accel-lzl-v:' . $lazyVideoCurId . '\',\'*\');', 'style' => array_merge( array( 'cursor' => 'pointer', 'position' => 'absolute', 'width' => '100%', 'height' => '100%' ), $thumbStyles ) ) ) .
...
function Frames_Process( &$ctxProcess, $doc, $settCache, $settFrm, $settImg, $settCdn, $settJs )
{
	...
	foreach( HtmlNd::ChildrenAsArr( $doc -> getElementsByTagName( 'iframe' ) ) as $item )
	{
		...
		if( !$isVideo || ( $isImgLazy && !Images_CheckLazyExcl( $ctxProcess, $doc, $settImg, $item ) ) )
		{
			...
			$item -> setAttribute( 'src', LazyLoad_SrcSubst( $ctxProcess, array( 'cx' => $sz ? $sz[ 0 ] : null, 'cy' => $sz ? $sz[ 1 ] : null ), Gen::GetArrField( $settImg, array( 'lazy', 'plchRast' ), true ) ) );
		}
...
function ContParts_GetAlwaysReplacements( &$a, $sett, $settContPr )
{
	$settCp = Gen::GetArrField( $settContPr, array( 'cp' ), array() );

	if( Gen::GetArrField( $settCp, array( 'wcs' ), false ) && defined( 'WOOCS_VERSION' ) )
	{

		$a[] = array( 'expr' => '@<script[^>]*>[^<]*woocs_default_currency@ & @<body[^>]*>()@', 'data' => "<script>(function(d,a){a.woocs_redirect_orig=a.woocs_redirect;a.woocs_redirect=function(b){var e=a.woocs_default_currency.name==b?\"\":b,c=new Date;c.setTime(c.getTime()+31536E6);d.cookie=\"wccs_changed_currency=\"+e+\";path=/;SameSite=Lax;expires=\"+c.toUTCString();a.woocs_redirect_orig(b)}})(document,window)</script>" );
	}
}
...
function ContParts_Process( &$ctxProcess, $doc, $settCache, $settCp, $settImg, $settFrm, $settCdn, $jsNotCritsDelayTimeout )
{
	...
	if( Gen::GetArrField( $settCp, array( 'wooOuPrdGal' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooOuPrdGal( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'wooGsPrdGal' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooGsPrdGal( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrBgSldshw' ), false ) )
	{
		...
		foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," elementor-element ")][contains(@data-settings,"background_slideshow_gallery")]' ) as $item )
		{
			...
			if( in_array( 'elementor-invisible', HtmlNd::GetAttrClass( $item ) ) )
				$cssSel .= ',' . $cssSel . '.elementor-invisible';
...
' );
	...
	if( Gen::GetArrField( $settCp, array( 'prstPlr' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		if( _ProcessCont_Cp_prstPlr( $ctx, $ctxProcess, $settFrm, $doc, $xpath ) === false )
			return( false );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'haCrsl' ), false ) )
	{
		...
		foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," ha-carousel ")][@data-settings]' ) as $item )
		{
			...
			if( ($dataSett[ 'navigation' ]??null) == 'dots' )
				_ProcessCont_Cp_slick_AddDots( $doc, $itemSlidesTmp, 'slick-dots', count( $aSlides ), function( $sld, $i ) { return( '<li class="" role="presentation"><button type="button" role="tab"></button></li>' ); } );
	...
	if( Gen::GetArrField( $settCp, array( 'wooPrdGallSld' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooPrdGallSld_2_3( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'wooPrdGall' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooPrdGall( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'wooPrdGallAstrThmbsHeight' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooPrdGallAstrThmbsHeight( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'wooPrdGallFltsmThmbs' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooPrdGallFltsmThmbs( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'wooPrdGallCrftThmbs' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooPrdGallCrftThmbs( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'wooNasaPrdGall' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_wooNasaPrdGall( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && Gen::GetArrField( $settCp, array( 'kpPsvStls' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );

		foreach( HtmlNd::ChildrenAsArr( $xpath -> query( './/style[@data-type="kppassive"][@data-kplinkhref] | .//link[@data-type="kppassive"][@rel="stylesheet"][@data-kplinkhref]' ) ) as $item )
		{
			$item = HtmlNd::SetTag( $item, 'link' );
			$item -> setAttribute( 'rel', 'stylesheet' );
			$item -> removeAttribute( 'data-type' );
			HtmlNd::RenameAttr( $item, 'data-kplinkhref', 'href' );
		}
	}
	...
	if( Gen::GetArrField( $settCp, array( 'diviMv' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_diviMv( $ctx, $ctxProcess, $settFrm, $settImg, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'diviVidBox2' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_diviVidBox2( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'diviCntdwnTmr' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_diviCntdwnTmr( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrWdgtCntdwn' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_elmntrWdgtCntdwn( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrWdgtTmFnfctCntr' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_elmntrWdgtTmFnfctCntr( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrRspnsv' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_elmntrRspnsv( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrWdgtAniHdln' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_elmntrWdgtAniHdln( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'elmntrStckHdr' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_elmntrStckHdr( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'xstrThSwpr' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_xstrThSwpr( $ctx, $ctxProcess, $settFrm, $settCache, $settImg, $settCdn, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'vhcThCarFldGall' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_vhcThCarFldGall( $ctx, $ctxProcess, $settFrm, $settCache, $settImg, $settCdn, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'bdeSwpr' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_bdeSwpr( $ctx, $ctxProcess, $settFrm, $settCache, $settImg, $settCdn, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'bdeTabs' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_bdeTabs( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
	...
	if( Gen::GetArrField( $settCp, array( 'btCntSld' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_btCntSld( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'btMenu' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_btMenu( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'slckGen' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_slckGen( Gen::GetArrField( $settCp, array( 'slckGen_a' ), array() ), $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'swprGen' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_swprGen( Gen::GetArrField( $settCp, array( 'swprGen_a' ), array() ), $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}

	if( Gen::GetArrField( $settCp, array( 'tpgbAni' ), false ) )
	{
		if( !$xpath )
			$xpath = new \DOMXPath( $doc );
		_ProcessCont_Cp_tpgbAni( $ctx, $ctxProcess, $settFrm, $doc, $xpath );
	}
...
function _Cp_XPathCssQuery( $ctx, $xpath, $sel, $item = null )
{
	if( !$ctx -> cnvCssSel2Xpath )
		$ctx -> cnvCssSel2Xpath = StyleProcessor::createCnvCssSel2Xpath();

	$sel = StyleProcessor::cssSelToXPathEx( $ctx -> cnvCssSel2Xpath, $sel );
	if( !$sel )
		return( null );

	return( $xpath -> query( $sel, $item ) );
}

function _Cp_GetScrCont( &$ctxProcess, $xpath, $query, $srcAttr = 'src' )
{
	$itemScr = HtmlNd::FirstOfChildren( $xpath -> query( $query ) );
	if( !$itemScr )
		return( false );
	return( _Cp_GetScrContEx( $ctxProcess, $itemScr, $srcAttr ) );
}

function _Cp_GetScrContEx( &$ctxProcess, $itemScr, $srcAttr = 'src' )
{
	if( !$itemScr )
		return( null );

	$src = $srcAttr ? $itemScr -> getAttribute( $srcAttr ) : null;
	if( !$src )
		return( $itemScr -> nodeValue );
	return( _Cp_GetScrContExEx( $ctxProcess, $src ) );
}

function _Cp_GetScrContExEx( &$ctxProcess, $src )
{
	$srcInfo = GetSrcAttrInfo( $ctxProcess, null, null, $src );
...
function _Cp_CloneStylesByExpr( $ctxProcess, $xpath, $aExpr, $cbAbort = null )
{
	$ctx = new AnyObj();
	$ctx -> aExpr = $aExpr;
	$ctx -> cb =
		function( $ctx, $sel, $bReplace = true, $bFull = false )
		{
			if( !$bReplace )
			{
				foreach( $ctx -> aExpr as $eF => $r )
	...
	return( _Cp_CloneStyles( $ctxProcess, $xpath, array( $ctx, 'cb' ) ) );
}

function _Cp_CloneStyles( $ctxProcess, $xpath, $cbSelector, $cbAbort = null )
{
	$contRes = '';
	foreach( $xpath -> query( './/link[@rel="stylesheet"][@href] | .//style' ) as $itemStyle )
	{
		$media = $itemStyle -> getAttribute( 'media' );
		if( $media == 'all' )
			$media = '';

		$cont = _Cp_GetScrContEx( $ctxProcess, $itemStyle, 'href' );

		$r = _Cp_CloneStylesEx( $cont, $cbSelector, $cbAbort );
		if( $r === false )
			return( false );
...
function _Cp_CloneStylesEx( $cont, $cbSelector, $cbAbort = null )
{
	$ctx = new AnyObj();
	$ctx -> cbSelector = $cbSelector;
	$ctx ->	cbAbort = $cbAbort;

	if( !call_user_func( $ctx -> cbSelector, ( string )$cont, false ) )
		return( '' );

	$cssParser = new Sabberworm\CSS\Parsing\ParserState( $cont, Sabberworm\CSS\Settings::create() -> withKeepComments( false ) -> withMultibyteSupport( false ) -> withLenientParsing( Sabberworm\CSS\Settings::ParseErrMed | Sabberworm\CSS\Settings::ParseErrHigh ) );
	$cssDoc = new Sabberworm\CSS\CSSList\Document( $cssParser -> currentPos() );
	try { $cssDoc -> parseEx( $cssParser ); } catch( \Exception $e ) { return( '' ); }

	$ctx ->	cssCritDoc = new Sabberworm\CSS\CSSList\Document();

	$blockParents = array();
	$blockParentsClone = array();

	if( _Cp_CloneStyles_Block( $ctx, $cssDoc, $blockParents, $blockParentsClone ) === false )
		return( false );

	$format = Sabberworm\CSS\OutputFormat::createPretty() -> set( 'Space*Rules', "\r\n" ) -> set( 'Space*Blocks', "\r\n" ) -> setSpaceBetweenBlocks( "\r\n\r\n" );

	return( $ctx ->	cssCritDoc -> renderWhole( $format ) );
}

function _Cp_CloneStyles_Block( $ctx, $block, &$blockParents, &$blockParentsClone, $bClone = false )
{
	if( !( $block instanceof Sabberworm\CSS\CSSList\CSSList ) )
		return;

	$blockParents[] = $block;
	$blockParentsClone[] = null;

	$selectorsClone = array();
	if( $block instanceof Sabberworm\CSS\RuleSet\DeclarationBlock )
	{
		foreach( $block -> getSelectors() as $sel )
		{
			$selNew = call_user_func( $ctx -> cbSelector, ( string )$sel, true );
			if( $selNew )
				$selectorsClone[] = $selNew;
			else if( $bClone )
				$selectorsClone[] = $sel;
		}
	...
	foreach( $block -> getContents() as $blockSub )
	{
		if( _Cp_CloneStyles_Block( $ctx, $blockSub, $blockParents, $blockParentsClone, $bClone ) === false )
			return( false );
		if( $ctx -> cbAbort && call_user_func( $ctx -> cbAbort ) ) return( false );
	}

	if( $bClone )
	{
		for( $iParent = 0; $iParent < count( $blockParentsClone ); $iParent++ )
		{
			if( $blockParentsClone[ $iParent ] )
				continue;

			$oParent = $blockParents[ $iParent ];
			$oParentClone = null;
			if( $oParent instanceof Sabberworm\CSS\CSSList\Document )
				$oParentClone = $ctx ->	cssCritDoc;
			else if( $oParent instanceof Sabberworm\CSS\RuleSet\DeclarationBlock )
			{
			...
			$blockParentsClone[ $iParent ] = $oParentClone;
			if( $iParent > 0 )
				$blockParentsClone[ $iParent - 1 ] -> append( $oParentClone );
		}

		if( $block instanceof Sabberworm\CSS\RuleSet\RuleSet )
			$blockParentsClone[ count( $blockParentsClone ) - 1 ] -> setRules( $block -> getRules() );
		if( $block instanceof Sabberworm\CSS\RuleSet\DeclarationBlock )
			$blockParentsClone[ count( $blockParentsClone ) - 1 ] -> setSelectors( $selectorsClone );
	}

	array_pop( $blockParents );
	array_pop( $blockParentsClone );
}
...
function _ProcessCont_Cp_jqSldNivo( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," nivoSlider ")]' ) as $item )
	{
		...
		if( $itemFirtSlide = HtmlNd::FirstOfChildren( $xpath -> query( '(.//img)[' . ( string )( Gen::GetArrField( $prms, array( 'startSlide' ), 0 ) + 1 ) . ']', $item ) ) )
		{
			...
			$sldAlt = ( string )$itemFirtSlide -> getAttribute( 'alt' );
		...
		if( $sldImgSrc )
		{
			...
			$item -> appendChild( HtmlNd::CreateTag( $doc, 'img', array( 'class' => array( 'nivo-main-image', 'js-lzl-ing' ), 'style' => array( 'display' => 'inline' ), 'src' => $sldImgSrc, 'alt' => $sldAlt ) ) );
...
function _ProcessCont_Cp_swiper_AdjustItem( $item, $itemSwpr, $aPrm, $ctx, &$ctxProcess, $doc, $xpath )
{
	$cmnStyle = '';

	$cssClassPagination = 'swiper-pagination';

	$direction = ($aPrm[ 'isVert' ]??false) ? 'vertical' : 'horizontal';
	$cssClass = ($aPrm[ 'cssClass' ]??'swiper-container');
	$cssClassWrapper = ($aPrm[ 'cssClassWrapper' ]??'swiper-wrapper');
	$cssSelPagination = ($aPrm[ 'cssSelPagination' ]??'.' . $cssClassPagination);

	$cssSelContainer = '.' . $cssClass;
	$cssSelContainerInited = '.' . ($aPrm[ 'cssClassInited' ]??'swiper-container-initialized');

	HtmlNd::AddRemoveAttrClass( $itemSwpr, array( $cssClass . '-' . $direction ) );

	$nItemsTotal = 0;
	if( $itemSwprWrp = HtmlNd::GetFirstElement( $itemSwpr ) )
		for( $itemSwprChild = HtmlNd::GetFirstElement( $itemSwprWrp ); $itemSwprChild; $itemSwprChild = HtmlNd::GetNextElementSibling( $itemSwprChild ) )
			$nItemsTotal++;

	$nItemsMax = ( int )($aPrm[ 'itemsMax' ]??0);
	foreach( $aPrm[ 'breakpoints' ] as $dim )
	{
		$nItems = ($dim[ 'slidesPerView' ]??0);
		if( $nItems == 'auto' )
			continue;
		...
		$media = Ui::StyleMediaMinMax( ($dim[ 'minWidth' ]??null), ($dim[ 'maxWidth' ]??null) );
		if( $media )
			$cmnStyle .= '@media ' . $media . ' { ';

		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . ' { --lzl-swpr-n: ' . ( string )$nItems . '; --lzl-swpr-gn: ' . ( string )( ( int )ceil( $nItemsTotal / $nItems ) ) . '; }';
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > .' . $cssClassWrapper . ' > *:nth-child(n+' . ( string )( $nItems + 1 ) . ') { visibility: hidden !important; }';

		if( $media )
			$cmnStyle .= ' }';
	}

	foreach( $aPrm[ 'breakpointsSpace' ] as $dim )
	{
		$media = Ui::StyleMediaMinMax( ($dim[ 'minWidth' ]??null), ($dim[ 'maxWidth' ]??null) );
		if( $media )
			$cmnStyle .= '@media ' . $media . ' { ';

		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . ' { --lzl-swpr-sps: ' . ( string )($dim[ 'space' ]??10) . 'px; }';

		if( $media )
			$cmnStyle .= ' }';
	}

	if( $itemNavPrev = HtmlNd::FirstOfChildren( _Cp_XPathCssQuery( $ctx, $xpath, $aPrm[ 'cssIdSelPrefix' ] . ' ' . ( string )($aPrm[ 'cssSelNavPrev' ]??'.swiper-button-prev'), $item ) ) )
		HtmlNd::AddRemoveAttrClass( $itemNavPrev, array( ( string )($aPrm[ 'cssClassNavDisabled' ]??'swiper-button-disabled') ) );

	if( ($aPrm[ 'isVert' ]??false) )
	{
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > .' . $cssClassWrapper . ' { flex-direction: column; }';
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > .' . $cssClassWrapper . ' > * { margin-bottom: var(--lzl-swpr-sps) !important; height: calc((var(--lzl-swpr-sz) - var(--lzl-swpr-sps) * (var(--lzl-swpr-n) - 1)) / var(--lzl-swpr-n)) !important; width1: 100% !important; }';
	}
	else
		$cmnStyle .= $aPrm[ 'cssIdSelPrefix' ] . $cssSelContainer . ':not(' . $cssSelContainerInited . ') > .' . $cssClassWrapper . ' > * { margin-right: var(--lzl-swpr-sps) !important; width: calc((var(--lzl-swpr-sz) - var(--lzl-swpr-sps) * (var(--lzl-swpr-n) - 1)) / var(--lzl-swpr-n)) !important; height1: 100% !important; }';

	$item -> setAttribute( 'style', Ui::GetStyleAttr( array_merge( Ui::ParseStyleAttr( $item -> getAttribute( 'style' ) ), array( '--lzl-swpr-sz' => '100%' ) ) ) );

	$paginationType = ($aPrm[ 'paginationType' ]??null);
	if( $paginationType && ( $itemPagination = HtmlNd::FirstOfChildren( _Cp_XPathCssQuery( $ctx, $xpath, $cssSelPagination, $item ) ) ) )
	{
		HtmlNd::AddRemoveAttrClass( $itemPagination, array( ($aPrm[ 'paginationClickable' ]??null) ? ( $cssClassPagination . '-clickable' ) : null, $cssClassPagination . '-' . $paginationType, $cssClassPagination . '-' . $direction ) );

		switch( $paginationType )
		{
		case 'bullets':
			for( $i = 0; $i < $nItemsTotal; $i++ )
				HtmlNd::Append( $itemPagination, HtmlNd::ParseAndImport( $doc, '<span class="' . $cssClassPagination . '-bullet' . ( $i == 0 ? ( ' ' . $cssClassPagination . '-bullet-active' ) : '' ) . '" tabindex="0" role="button" aria-label="Go to slide ' . ( $i + 1 ) . '" aria-current="true"></span>' ) );
			break;

		case 'progressbar':
			$scale = 'calc(1 / var(--lzl-swpr-gn))';
			if( ($aPrm[ 'isVert' ]??false) )
			{
				$scaleX = '1';
				$scaleY = $scale;
			...
			HtmlNd::Append( $itemPagination, HtmlNd::ParseAndImport( $doc, '<span class="' . $cssClassPagination . '-progressbar-fill" style="transform: translate3d(0px, 0px, 0px) scaleX(' . $scaleX . ') scaleY(' . $scaleY . ');"></span>' ) );
			break;
		}
	}
...
function _ProcessCont_Cp_swprGen( $a, $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$aItemStyleCont = array();
	...
	foreach( $a as $ai )
	{
		...
		foreach( _Cp_XPathCssQuery( $ctx, $xpath, $selCss ) as $item )
		{
			if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
				continue;
			...
			$aPrm = array(
				'breakpoints' => array(),
				'breakpointsSpace' => array(),
				'cssClass' => 'swiper',
				'cssClassInited' => 'swiper-initialized',
				'cssIdSelPrefix' => $selCss,

			);

			HtmlNd::AddRemoveAttrClass( $item, array( 'lzl-js', 'swpr-ed-lzl' ) );

			if( Gen::GetArrField( $ai, array( 'pagination', 'enable' ), false ) )
			{
				$aPrm[ 'paginationType' ] = Gen::GetArrField( $ai, array( 'pagination', 'type' ), 'bullets' );
				$aPrm[ 'cssSelPagination' ] = Gen::GetArrField( $ai, array( 'pagination', 'selCss' ) );
				$aPrm[ 'paginationClickable' ] = Gen::GetArrField( $ai, array( 'pagination', 'clickable' ), false );
			}

			foreach( ($ai[ 'bps' ]??array()) as $bp )
			{
				$aPrm[ 'breakpoints' ][] = array( 'minWidth' => ($bp[ 'minWidth' ]??null), 'maxWidth' => ($bp[ 'maxWidth' ]??null), 'slidesPerView' => ($bp[ 'slidesPerView' ]??null) );
				$aPrm[ 'breakpointsSpace' ][] = array( 'minWidth' => ($bp[ 'minWidth' ]??null), 'maxWidth' => ($bp[ 'maxWidth' ]??null), 'space' => ($bp[ 'spaceBetween' ]??null) );
			}

			$itemStyleCont .= _ProcessCont_Cp_swiper_AdjustItem( $item, $item, $aPrm, $ctx, $ctxProcess, $doc, $xpath );
		...
		{
			{
				$aItemStyleCont[] = $itemStyleCont;
			}
		}
	}

	if( $aItemStyleCont && ( $ctxProcess[ 'mode' ] & 1 ) )
	{
		$ctxProcess[ 'aCssRpl' ][ '@(\\.swiper-initialized)@' ] = ':is(.swiper-initialized,.swpr-ed-lzl)';

		foreach( $aItemStyleCont as $itemStyleCont )
		{
			$itemCmnStyle = $doc -> createElement( 'style' );
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemCmnStyle -> setAttribute( 'type', 'text/css' );
			HtmlNd::SetValFromContent( $itemCmnStyle, $itemStyleCont );
			$ctxProcess[ 'ndHead' ] -> appendChild( $itemCmnStyle );

			$ctxProcess[ 'aCssRplExcl' ][] = $itemCmnStyle;
		}
	}
}

function _ProcessCont_Cp_slick_PrepareCont( $ctx, $doc, $xpath, $itemSlides, $classSlide, $cbSlide = null )
{
	if( !$itemSlides )
		return( null );
	...
	{
		if( $classSlide !== null && !in_array( $classSlide, HtmlNd::GetAttrClass( $itemSlide ) ) )
		{
...

		{
			...
			if( $sld -> nRows > 1 )
				$itemSlide = HtmlNd::CreateTag( $doc, 'div', array( 'class' => array( 'slick-slide' ) ), array( HtmlNd::CreateTag( $doc, 'div', array(), array( $itemSlide ) ) ) );
			if( $cbSlide )
				$itemSlide = $cbSlide( $sld, $doc, $itemSlide );
			$itemSlidesContTmp -> appendChild( $itemSlide );
		}

		if( $sld -> nSlides == 1 )
			HtmlNd::AddRemoveAttrClass( $itemSlide, array( 'slick-current', 'slick-active' ) );
	}
...
function _ProcessCont_Cp_slick_GetGlobStyle( $selSlides, $classSlide, $adaptiveHeight = false )
{
	return( '' . $selSlides . '.slick-initialized + ' . $selSlides . '.js-lzl-ing,
...
function _ProcessCont_Cp_slick_InitGlob( $ctx, &$ctxProcess, $doc, $selSlides )
{
	...
	if( $ctxProcess[ 'bJsCssAddType' ] )
		$itemScript -> setAttribute( 'type', 'text/javascript' );
	$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
	HtmlNd::SetValFromContent( $itemScript, str_replace( '_PRM_SELSLIDES_', $selSlides, "(function(d,c){seraph_accel_izrbpb.add(function(){d.querySelectorAll(\"_PRM_SELSLIDES_:not(.slick-initialized):not(.lzl-c):not(.js-lzl-ing)\").forEach(function(a){c.MutationObserver&&(a.obsedgvsd=new c.MutationObserver(function(){if(a.obsedgvsd&&a.classList.contains(\"slick-initialized\")){a.obsedgvsd.disconnect();delete a.obsedgvsd;a.slick&&a.slick.refresh();var b=a.parentNode.querySelector(\".js-lzl-ing\");b&&a.parentNode===b.parentNode&&a.parentNode.removeChild(b)}}),a.obsedgvsd.observe(a,{attributes:!0,\nattributeFilter:[\"class\"]}))})},110)})(document,window)" ) );
	$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
}

function _ProcessCont_Cp_slick_AddDots( $doc, $itemSlides, $class, $n, $cbItemTpl, $cbCtx = null )
{
	HtmlNd::AddRemoveAttrClass( $itemSlides, array( 'slick-dotted' ) );

	$itemCtl = HtmlNd::ParseAndImport( $doc, '<ul class="' . implode( ' ', ( array )$class ) . '" role="tablist"></ul>' );
	if( !$itemCtl )
		return;
...
function _ProcessCont_Cp_slick_GetDotsCount( $aPrm = array( 'slideCount' => 1, 'slidesToShow' => 1, 'slidesToScroll' => 1, 'infinite' => false, 'centerMode' => false, 'asNavFor' => false ) )
{
	$e = 0;
	$t = 0;
	$o = 0;
...
function _ProcessCont_Cp_slick_GetObjKey( $v )
{
	if( $v instanceof \DOMElement )
		return( $v -> textContent );
	return( Gen::VarExport( $v, array( 'nonScalar' => true, 'indent' => '', 'elemSpace' => '', 'assignSpaceBefore' => '', 'assignSpaceAfter' => '', 'escValNl' => true ) ) );
}

function _ProcessCont_Cp_slckGen( $a, $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$aItemStyleCont = array();
	...
	foreach( $a as $ai )
	{
		...
		foreach( _Cp_XPathCssQuery( $ctx, $xpath, $selCss ) as $item )
		{
			...
			$sld = _ProcessCont_Cp_slick_PrepareCont( $ctx, $doc, $xpath, $item, null,
				function( $sld, $doc, $itemSlide )
				{
					HtmlNd::AddRemoveAttrClass( $itemSlide, array( 'slick-slide' ) );
					return( $itemSlide );
				}
			);
			if( !$sld )
			...
			HtmlNd::AddRemoveAttrClass( $sld -> itemSlides, array( 'slick-slider', 'slck-ed-lzl' ) );

			$prevArrowCmn = null;
			$nextArrowCmn = null;
			$customPagingCmn = null;
			$aDotsClassCmn = ( array )Gen::GetArrField( $ai, array( 'dotsClass' ), 'slick-dots', '', false, false );

			$alignItems = Gen::GetArrField( $ai, array( 'alignItems' ) );
			if( $alignItems )
			...
			$aDots = array();
			$dimMin = 0;
			foreach( Gen::GetArrField( $ai, array( 'bps' ), array() ) as $dim => $aiR )
			{
				$nShow = ( int )Gen::GetArrField( $aiR, array( 'slidesToShow' ), 0 );
				if( $nShow < 1 )
					$nShow = 1;

				if( $nShow > $sld -> nSlides )
					$nShow = $sld -> nSlides;

				$media = Ui::StyleMediaMinMax( $dimMin, $dim );
				if( $media )
					$itemStyleCont .= '@media ' . $media . ' { ';

				if( Gen::GetArrField( $aiR, array( 'adaptiveHeight' ), false ) )
				{
					if( Gen::GetArrField( $aiR, array( 'vertical' ), false ) )
					{
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c { ' . $alignItems . 'display: flex; flex-direction: column; }' . "\n";
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c > * { height: 100%; }' . "\n";
					}
					else
					{
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c { ' . $alignItems . 'display: flex; }' . "\n";
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c > * { width: 100%; }' . "\n";
					}
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c > *:nth-child(n+' . ( $nShow + 1 ) . ') {display:none!important;}' . "\n";
				}
				else
				{
					if( Gen::GetArrField( $aiR, array( 'vertical' ), false ) )
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c { ' . $alignItems . 'display: grid; grid-template-rows:repeat(' . $nShow . ', calc(100%/' . $nShow . ')); }' . "\n";
					else
						$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c { ' . $alignItems . 'display: grid; grid-template-columns:repeat(' . $nShow . ', calc(100%/' . $nShow . ')); }' . "\n";
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c > *:nth-child(n+' . ( $nShow + 1 ) . ') {visibility:hidden!important;grid-column:1;grid-row:1;}' . "\n";
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c > *:first-child { grid-column: 1; grid-row: 1; }';
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .lzl-c:before, ' . $selCss . '.slick-slider:not(.slick-initialized) .lzl-c:after { display: none !important; }';
				}

				{
					$nDots = _ProcessCont_Cp_slick_GetDotsCount( array( 'slideCount' => $sld -> nSlides, 'slidesToShow' => $nShow, 'slidesToScroll' => Gen::GetArrField( $aiR, array( 'slidesToScroll' ), 1 ), 'infinite' => Gen::GetArrField( $aiR, array( 'infinite' ), true ), 'centerMode' => Gen::GetArrField( $aiR, array( 'centerMode' ), false ), 'asNavFor' => false ) );
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .' . implode( '.', $aDotsClassCmn ) . ( $nDots ? ' > *:nth-child(n+' . ( $nDots + 1 ) . ')' : '' ) . ' {display:none;}' . "\n";
				}

				if( Gen::GetArrField( $aiR, array( 'arrows' ), true ) )
				{
					if( $prevArrowCmn === null )
						$prevArrowCmn = Gen::GetArrField( $ai, array( 'prevArrow' ), '<button class="slick-prev" aria-label="Previous" type="button">Previous</button>', '', false, false );
					if( $prevArrow = ($aiR[ 'prevArrow' ]??$prevArrowCmn) )
					{
						$info = &$aArrows[ 'P:' . _ProcessCont_Cp_slick_GetObjKey( $prevArrow ) ];
						$info[ 'v' ] = $prevArrow;
						$info[ 'bps' ][] = $dim;
						unset( $info );
					}

					if( $nextArrowCmn === null )
						$nextArrowCmn = Gen::GetArrField( $ai, array( 'nextArrow' ), '<button class="slick-next" aria-label="Next" type="button">Next</button>', '', false, false );
					if( $nextArrow = ($aiR[ 'nextArrow' ]??$nextArrowCmn) )
					{
						$info = &$aArrows[ 'N:' . _ProcessCont_Cp_slick_GetObjKey( $nextArrow ) ];
						$info[ 'v' ] = $nextArrow;
						$info[ 'bps' ][] = $dim;
						unset( $info );
					}
					...
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .slick-arrow:not(.bp-' . $dim . ') {display:none;}' . "\n";

				if( Gen::GetArrField( $aiR, array( 'dots' ), false ) )
				{
					if( $customPagingCmn === null )
						$customPagingCmn = Gen::GetArrField( $ai, array( 'customPaging' ), '<li><button type="button">{{SLIDE_NUMBER}}</button></li>', '', false, false );

					$customPaging = ($aiR[ 'customPaging' ]??$customPagingCmn);
					$info = &$aDots[ _ProcessCont_Cp_slick_GetObjKey( $customPaging ) ];
					$info[ 'v' ] = $customPaging;
					$info[ 'bps' ][] = $dim;
					unset( $info );
				}
				else
					$itemStyleCont .= $selCss . '.slick-slider:not(.slick-initialized) .' . implode( '.', $aDotsClassCmn ) . ':not(.bp-' . $dim . ') {display:none;}' . "\n";
			...
			foreach( $aArrows as $info )
			{
				$itemArrow = $info[ 'v' ];
				if( is_string( $itemArrow ) )
					$itemArrow = HtmlNd::ParseAndImport( $doc, $itemArrow );
				if( $itemArrow )
				{
					$aClass = array( 'slick-arrow' );
					foreach( $info[ 'bps' ] as $dim )
						$aClass[] = 'bp-' . $dim;

					HtmlNd::AddRemoveAttrClass( $itemArrow, $aClass );
					$sld -> itemSlides -> insertBefore( $itemArrow, $sld -> itemSlides -> firstChild );
				}
			}

			foreach( $aDots as $info )
			{
				$cbCustomPaging = $info[ 'v' ];
				if( is_string( $cbCustomPaging ) )
				{
					$ctxCustomPaging = new AnyObj();
					$ctxCustomPaging -> dotTpl = ( string )$cbCustomPaging;
					$ctxCustomPaging -> cb = function( $ctxCustomPaging, $sld, $i ) { return( Gen::StrReplaceKeyed( array( '{{SLIDE_NUMBER}}' => ( string )( $i + 1 ) ), $ctxCustomPaging -> dotTpl ) ); };
					$cbCustomPaging = array( $ctxCustomPaging, 'cb' );
				}

				$aClass = $aDotsClassCmn;
				foreach( $info[ 'bps' ] as $dim )
					$aClass[] = 'bp-' . $dim;

				_ProcessCont_Cp_slick_AddDots( $doc, $sld -> itemSlides, $aClass, $sld -> nSlides, $cbCustomPaging, $sld );
			}
		...
		{
			{
				$itemStyleCont .= _ProcessCont_Cp_slick_GetGlobStyle( $selCss, '' );

				$aItemStyleCont[] = $itemStyleCont;
			}

			_ProcessCont_Cp_slick_InitGlob( $ctx, $ctxProcess, $doc, $selCss );
		}
	}

	if( $aItemStyleCont && ( $ctxProcess[ 'mode' ] & 1 ) )
	{
		$ctxProcess[ 'aCssRpl' ][ '@(\\.slick-initialized)@' ] = ':is(.slick-initialized,.slck-ed-lzl)';
...
function _ProcessCont_Cp_diviMv( $ctx, &$ctxProcess, $settFrm, $settImg, $doc, $xpath )
{
	$adjusted = false;
	foreach( $xpath -> query( './/*[@data-et-multi-view]' ) as $item )
	{
		...
		if( $item -> nodeName == 'img' && !$ctxProcess[ 'isAMP' ] && Gen::GetArrField( $settImg, array( 'lazy', 'load' ), false ) && Gen::GetArrField( $settImg, array( 'lazy', 'own' ), false ) )
		{
			$exclMode = Images_CheckLazyExcl( $ctxProcess, $doc, $settImg, $item );
			if( !( $exclMode && $exclMode !== 'ajs' ) )
			{
				$dataSett = ( array )@json_decode( $item -> getAttribute( 'data-et-multi-view' ), true );

				foreach( Gen::GetArrField( $dataSett, array( 'schema', 'attrs' ), array() ) as $screenMode => $data )
				{
					Gen::RenameArrField( $data, 'srcset', 'data-lzl-srcset' );
					Gen::RenameArrField( $data, 'sizes', 'data-lzl-sizes' );

					$src = Gen::GetArrField( $data, 'src' );
					$imgSrc = new ImgSrc( $ctxProcess, $src );
					Gen::SetArrField( $data, 'data-lzl-src', $src );
					Gen::SetArrField( $data, 'src', LazyLoad_SrcSubst( $ctxProcess, $imgSrc && ( ( $ctxProcess[ 'mode' ] & 2 ) || $imgSrc -> IsCont() ) ? $imgSrc -> GetInfo() : null, Gen::GetArrField( $settImg, array( 'lazy', 'plchRast' ), true ) && !!( $ctxProcess[ 'mode' ] & 2 ) ) );

					Gen::SetArrField( $data, 'class', Ui::AddRemoveAttrClass( Gen::GetArrField( $data, 'class', '' ), array( 'lzl', $exclMode === 'ajs' ? ' bjs' : null ) ) );

					Gen::SetArrField( $dataSett, array( 'schema', 'attrs', $screenMode ), $data );
				}

				$item -> setAttribute( 'data-et-multi-view-lzl', ( string )json_encode( $dataSett ) );
			}
		}

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, 'seraph_accel_cp_diviMv_calcSizes_init(document.currentScript)' );
			HtmlNd::InsertAfter( $item -> parentNode, $itemScript, $item );
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
...

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "function seraph_accel_cp_diviMv_calcSizes_init(a){var b=a.previousElementSibling;a.parentNode.removeChild(a);\"NOSCRIPT\"==b.tagName&&(b=b.previousElementSibling);seraph_accel_cp_diviMv_calcSizes(b)}\nfunction seraph_accel_cp_diviMv_calcSizes(a){function b(d,f){return void 0!==f[d]?d:\"tablet\"==d?void 0!==f.desktop?\"desktop\":\"phone\":\"phone\"==d?void 0!==f.desktop?\"tablet\":\"desktop\":void 0!==f.desktop?\"tablet\":\"phone\"}var e=function(){var d=document.documentElement.clientWidth;return 981<=d?\"desktop\":768<=d?\"tablet\":\"phone\"}(),c;try{(c=a.getAttribute(\"data-et-multi-view-lzl\"))||(c=a.getAttribute(\"data-et-multi-view\")),c=JSON.parse(c)}catch(d){}let h,k;var g=null==(h=c)?void 0:null==(k=h.schema)?void 0:\nk.content;g&&(a.innerHTML=g[b(e,g)]);let l,m,n;c=null!=(n=null==(l=c)?void 0:null==(m=l.schema)?void 0:m.attrs)?n:{};if((e=c[b(e,c)])&&\"object\"===typeof e)for(var p in e)a.setAttribute(p,e[p]);a.classList.add(\"et_multi_view_swapped\")}\n(function(a){function b(e){a.querySelectorAll(\"[data-et-multi-view]:not(.et_multi_view_swapped)\").forEach(seraph_accel_cp_diviMv_calcSizes)}a.addEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){a.removeEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0})})})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	}
}
...
function _ProcessCont_Cp_diviVidBox2( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$adjusted = false;
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," et_pb_video_box ")]/iframe' ) as $item )
	{
		...
		$item -> setAttribute( 'style', Ui::GetStyleAttr( array_merge( Ui::ParseStyleAttr( $item -> getAttribute( 'style' ) ), array( '--width' => $item -> getAttribute( 'width' ), '--height' => $item -> getAttribute( 'height' ) ) ) ) );
		HtmlNd::AddRemoveAttrClass( $item, array( 'fitvidsignore' ) );
		$adjusted = true;
	}
...
function _ProcessCont_Cp_diviStck( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	{
		$item = HtmlNd::FirstOfChildren( _Cp_XPathCssQuery( $ctx, $xpath, Gen::GetArrField( $cfgItem, array( 'selector' ), '' ) ) );
		if( !$item )
			continue;
...
function _ProcessCont_Cp_diviCntdwnTmr( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$adjusted = false;
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," et_pb_countdown_timer ")]' ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		...
		$ctx -> aAniAppear[ '.et_pb_countdown_timer:not(.lzl-cntr-ad)' ] = "function(b){function e(){function d(f,a,g){f&&(a=String(a),a.length<g&&(a=\"0\".repeat(g-a.length)+a),f.textContent=a)}var c=k-Date.now();0>=c&&(c=0,clearInterval(h));d(l,Math.floor(c/864E5),3);d(m,Math.floor(c/36E5%24),2);d(n,Math.floor(c/6E4%60),2);d(p,Math.floor(c/1E3%60),2)}b.classList.add(\"lzl-cntr-ad\");var k=1E3*parseInt(b.getAttribute(\"data-end-timestamp\"),10),l=b.querySelector(\".days .value\"),m=b.querySelector(\".hours .value\"),n=b.querySelector(\".minutes .value\"),p=b.querySelector(\".seconds .value\");\ne();var h=setInterval(e,1E3);seraph_accel_izrbpb.add(function(){clearInterval(h)})}";
	}
}
...
function _ProcessCont_Cp_sldRev7( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		...
		$ctxProcess[ 'aJsCrit' ][ '!src:@.+@ & body:@(?:^|\\W)SR7\\.\\w@' ] = true;
	}
}
...
function _ProcessCont_Cp_elmntrStckHdr( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$adjusted = false;
	foreach( $xpath -> query( './/*[@id="wrapper"][count(.//*[@id="elementor-sticky-header"]) > 0]//*[@id="elementor-header"]' ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, '(function(s){seraph_accel_cp_elmntrStckHdr_calcSizes(s.parentNode);s.parentNode.removeChild(s);})(document.currentScript)' );
			$item -> appendChild( $itemScript );
		}
	...
	{
		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "function seraph_accel_cp_elmntrStckHdr_calcSizes(a){var b=document.querySelector(\"#wrapper\");b&&b.style.setProperty(\"padding-top\",\"\"+Math.round(a.getBoundingClientRect().height)+\"px\")}(function(a){function b(d){a.querySelectorAll(\"#wrapper #elementor-header\").forEach(function(c){seraph_accel_cp_elmntrStckHdr_calcSizes(c)})}a.addEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){a.removeEventListener(\"seraph_accel_calcSizes\",b,{capture:!0,passive:!0})})})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	}
}
...
function _ProcessCont_Cp_elmntrPremCrsl( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," premium-carousel-wrapper ")][@data-settings]' ) as $item )
	{
		...
		$ai = array(
			'enable' => true,
			'selCss' => $selId . '.premium-carousel-wrapper .premium-carousel-inner',
		);
		...
		if( ($dataSett[ 'arrows' ]??null) )
		{
			if( $itemPrev = HtmlNd::FirstOfChildren( $xpath -> query( './*[contains(concat(" ",normalize-space(@class)," ")," premium-carousel-nav-arrow-prev ")]/*[1]', $item ) ) )
				$ai[ 'prevArrow' ] = $itemPrev -> cloneNode( true );
			...
			if( $itemNext = HtmlNd::FirstOfChildren( $xpath -> query( './*[contains(concat(" ",normalize-space(@class)," ")," premium-carousel-nav-arrow-next ")]/*[1]', $item ) ) )
				$ai[ 'nextArrow' ] = $itemNext -> cloneNode( true );
		}

		if( ($dataSett[ 'dots' ]??null) )
		{
			$ctxCustomPaging = new AnyObj();
			$ctxCustomPaging -> cb = function( $ctxCustomPaging, $sld, $i ) { return( '<li role="presentation">' . $ctxCustomPaging -> dotTpl . '</li>' ); };

			$ctxCustomPaging -> dotTpl = '';
			if( ($dataSett[ 'carouselNavigation' ]??null) == 'dots' )
				$ctxCustomPaging -> dotTpl = ( string )HtmlNd::DeParse( HtmlNd::FirstOfChildren( $xpath -> query( './*[contains(concat(" ",normalize-space(@class)," ")," premium-carousel-nav-dot ")]', $item ) ), false );

			if( !$ctxCustomPaging -> dotTpl )
				$ctxCustomPaging -> dotTpl = 'X';

			$ai[ 'customPaging' ] = array( $ctxCustomPaging, 'cb' );
		}
		...
		foreach( $aViews as $optId => $maxWidth )
		{
			...
			$ai[ 'bps' ][ $maxWidth ] = array(
				'slidesToShow' => $nShow,
				'slidesToScroll' => ( int )($dataSett[ 'slidesToScroll' ]??null),
				'infinite' => ( bool )($dataSett[ 'infinite' ]??null),
				'centerMode' => ( bool )($dataSett[ 'centerMode' ]??null),
				'asNavFor' => false,
				'arrows' => !!($dataSett[ 'arrows' ]??null),
				'dots' => !!($dataSett[ 'dots' ]??null),
			);
		}
	...
	_ProcessCont_Cp_slckGen( $a, $ctx, $ctxProcess, $settFrm, $doc, $xpath );
}
...
function _ProcessCont_Cp_elmntrWdgtCntr( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		...
		$ctx -> aAniAppear[ '.elementor-widget-counter:not(.lzl-cntr-ad)' ] = 'function(c){var a=c.querySelector(".elementor-counter-number");if(a){c.classList.add("lzl-cntr-ad");c=parseInt(a.getAttribute("data-duration"),10);var b=parseInt(a.getAttribute("data-from-value"),10),d=parseInt(a.getAttribute("data-to-value"),10),f=a.getAttribute("data-delimiter");a.setAttribute("data-from-value",d);a.setAttribute("data-duration",0);var h=(d-b)/(c/10),g=setInterval(function(){b+=h;b>=d&&(b=d,clearInterval(g));var e=""+Math.round(b);f&&(e=e.replace(/\B(?=(\d{3})+(?!\d))/g,
f));a.textContent=e},10);seraph_accel_izrbpb.add(function(){clearInterval(g)})}}';
	}
}
...
function _ProcessCont_Cp_elmntrWdgtCntdwn( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		...
		if( $ctxProcess[ 'bJsCssAddType' ] )
			$itemScript -> setAttribute( 'type', 'text/javascript' );
		$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
		HtmlNd::SetValFromContent( $itemScript, "function seraph_accel_cp_elmntrWdgtCntdwn_Init(c){function l(){function e(b,a){function d(m,h){m&&(h=parseInt(h,10),m.innerText=10>h?\"0\"+h:h)}d(b.querySelector(\".elementor-countdown-digits.elementor-countdown-days\"),a/60/60/24%365);d(b.querySelector(\".elementor-countdown-digits.elementor-countdown-hours\"),a/60/60%24);d(b.querySelector(\".elementor-countdown-digits.elementor-countdown-minutes\"),a/60%60);d(b.querySelector(\".elementor-countdown-digits.elementor-countdown-seconds\"),a%60)}var f=Date.now();\nk<=f?(e(g,0),clearInterval(n)):e(g,Math.round((k-f)/1E3))}function p(e,f){var b=e.getAttribute(\"data-id\");e=b+\"-evergreen_due_date\";b+=\"-evergreen_interval\";var a=localStorage.getItem(e),d=localStorage.getItem(b);if(null===a&&null===d||null!==a&&f!==parseInt(d,10))return a=new Date,a=a.setSeconds(a.getSeconds()+f),localStorage.setItem(e,a),localStorage.setItem(b,f),a;if(0<a&&parseInt(d,10)===f)return a}c.classList.add(\"lzl-cntr-ed\");var g=c.querySelector(\".elementor-countdown-wrapper\");if(g){var k=\n1E3*parseInt(g.getAttribute(\"data-date\"),10);c=parseInt(g.getAttribute(\"data-evergreen-interval\"),10);0<c&&(k=p(g,c));k=new Date(k);l();var n=setInterval(l,1E3);seraph_accel_izrbpb.add(function(){clearInterval(n)})}}(function(c){c.addEventListener(\"seraph_accel_freshPartsDone\",function(){c.querySelectorAll(\".elementor-widget-countdown:not(.lzl-cntr-ed)\").forEach(function(l){seraph_accel_cp_elmntrWdgtCntdwn_Init(l)})},{capture:!0,passive:!0})})(document)" );
		$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
	}
}
...
function _ProcessCont_Cp_elmntrWdgtTmFnfctCntr( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		...
		$ctx -> aAniAppear[ '.elementor-widget-tm-ele-funfact-counter:not(.lzl-cntr-ad)' ] = 'function(b){var a=b.querySelector(".animate-number");if(a){b.classList.add("lzl-cntr-ad");b=parseInt(a.textContent,10)||0;var f=parseInt(a.dataset.value,10)||0,g=parseInt(a.dataset.animationDuration,10)||1E3;(function(h,d,k,l){function e(c){c=Math.min((c-m)/l,1);h.textContent=Math.floor(d+(k-d)*c);1>c&&requestAnimationFrame(e)}const m=performance.now();requestAnimationFrame(e)})(a,b,f,g)}}';
	}
}
...
function _ProcessCont_Cp_elmntrRspnsv( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$adjusted = false;
	if( $itemScr = HtmlNd::FirstOfChildren( $xpath -> query( './/script[contains(@src,"/wp-content/plugins/elementor/assets/js/frontend")]' ) ) )
	{
		$ctxProcess[ 'ndBody' ] -> appendChild( HtmlNd::CreateTag( $doc, 'span', array( 'id' => 'elementor-device-mode', 'class' => array( 'elementor-screen-only', 'js-lzl-ing' ) ) ) );
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		$ctxProcess[ 'aCssCrit' ][ '@\\[data-elementor-device-mode\\]@' ] = true;

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "(function(b){function c(a){(a=b.querySelector(\"#elementor-device-mode.elementor-screen-only\"))&&(a=getComputedStyle(a,\":after\").content.replace(/\"/g,\"\"))&&b.body.setAttribute(\"data-elementor-device-mode\",a)}b.addEventListener(\"seraph_accel_calcSizes\",c,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){b.removeEventListener(\"seraph_accel_calcSizes\",c,{capture:!0,passive:!0});var a=b.querySelector(\"#elementor-device-mode.elementor-screen-only.js-lzl-ing\");a&&a.parentNode.removeChild(a)})})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	}
}
...
function _ProcessCont_Cp__jetCrsl( $ctx, &$ctxProcess, $settFrm, $doc, $xpath, array $aPrm )
{
	...
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," ' . $aPrm[ 'item' ] . ' ")][@data-settings]' ) as $item )
	{
		...
		$ai = array(
			'enable' => true,
			'selCss' => '.' . $aPrm[ 'item' ] . ' .' . $classId . '.jet-carousel .' . $aPrm[ 'itemSlk' ],
			'dotsClass' => 'jet-slick-dots',
			'customPaging' => '<li tabindex="0"><span>{{SLIDE_NUMBER}}</span></li>',
		);
		...
		foreach( $aViews as $viewId => $view )
		{
			...
			$ai[ 'bps' ][ $maxWidth ] = array(
				'slidesToShow' => $nShow,
				'slidesToScroll' => ( int )($dataSettJet[ 'slidesToScroll' ]??null),
				'infinite' => ( bool )($dataSettJet[ 'infinite' ]??null),
				'centerMode' => false,
				'asNavFor' => false,
				'dots' => ($dataSettJet[ 'dots' ]??null),
			);
		}
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && ( $adjusted || $adjustedPl ) )
	{
		...
		if( $adjustedPl )
		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "(function(a){function b(e){a.querySelectorAll(\"presto-playlist.js-lzl:not(.lzl-ready)\").forEach(function(c){c.addEventListener(\"playerReady\",function(){var d=this;setTimeout(function(){d.classList.add(\"lzl-ready\")},500)},{capture:!0,passive:!0})})}seraph_accel_izrbpb.add(function(){b()},200)})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	}
}
...
function _PrstPlr_GetPrmsCmnFromScr( &$ctxProcess, $itemInitScr )
{
	...
	$url = Gen::GetArrField( $prms, array( 'prestoComponents', 'url' ) );
	$srcInfo = GetSrcAttrInfo( $ctxProcess, null, null, $url );
	if( ($srcInfo[ 'filePath' ]??null) )
	{
		$a = $ctxProcess[ 'cbs' ] -> PreFetchLocalFiles( array( Gen::GetFileDir( Gen::GetNormalizedPath( ($srcInfo[ 'filePath' ]??null) ) ) . '/*.js' => array( 'filePathRoot' => $srcInfo[ 'filePathRoot' ], 'fileNameFilter' => '@^p-[\\w\\-]+(?:\\.entry|)$@' ) ) );
		if( $a === false )
			return( false );

		foreach( $a as $filePath )
			$prms[ '_aJs' ][ str_replace( '\\', '/', $filePath ) ] = ( string ) $ctxProcess[ 'cbs' ] -> ReadLocalFile( $filePath, ($srcInfo[ 'filePathRoot' ]??null) );
...
function _PrstPlr_GetPrmsCmnCompCss( $prms, $id )
{
	...
	for( ;; )
	{
		...
		foreach( $aFile as $file )
		{
			...
			$dir = null;
			foreach( array(
				array( '@\\W\\w+\\s*=\\s*(([\'"]).*(?2))\\s*,\\s*\\w+\\s*=\\s*\\w+\\s*,\\s*' . preg_quote( $key ) . '\\s*=\\s*class\\s*{@', -1 ),
				array( '@\\W\\w+\\s*=\\s*(([\'"]).*(?2))\\s*,\\s*' . preg_quote( $key ) . '\\s*=\\s*class\\s*{@', -1 ),
				array( '@\\W' . preg_quote( $key ) . '\\s*\\.\\s*style\\s*=\\s*(([\'"]).*(?2))\\s*;@', 1 ),
...
function _ProcessCont_Cp_wooPrdGallSld_2_3( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{

	$aPrm = null;
	$a = array();

	foreach( HtmlNd::ChildrenAsArr( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," woocommerce-product-gallery ")][contains(concat(" ",normalize-space(@class)," ")," woo-product-gallery-slider ")]' ) ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;
		...
		if( $aPrm === null )
		{
			$aPrm = _ProcessCont_Cp_wooPrdGallSld_GetCmnPrms( $ctxProcess, $xpath );
			if( !$aPrm )
				return;
		}

		if( $itemVariationsForm = HtmlNd::FirstOfChildren( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," variations_form ")]' ) ) )
		{
			if( $itemVariationSelector = HtmlNd::FirstOfChildren( $xpath -> query( './/select[@data-attribute_name]', $itemVariationsForm ) ) )
			{
				if( $itemVariation = HtmlNd::FirstOfChildren( $xpath -> query( './option[@selected]', $itemVariationSelector ) ) )
				{
					$varAttrName = $itemVariationSelector -> getAttribute( 'data-attribute_name' );
					$varAttr = $itemVariation -> getAttribute( 'value' );

					$variationData = Gen::GetArrField( @json_decode( ( string )$itemVariationsForm -> getAttribute( 'data-product_variations' ), true ), array( '' ), array() );

					$varCont = null;
					foreach( $variationData as $vdi )
					{
						if( Gen::GetArrField( $vdi, array( 'attributes', $varAttrName ) ) != $varAttr )
							continue;

						$varCont = Gen::GetArrField( $aPrm, array( 'variation_data', Gen::GetArrField( $vdi, array( 'variation_id' ), 0 ) ) );
						break;
					}

					if( $varCont && $itemVar = HtmlNd::ParseAndImport( $doc, $varCont ) )
					{
						HtmlNd::AddRemoveAttrClass( $itemVar, array( 'woo-product-gallery-slider' ) );
						$item -> parentNode -> insertBefore( $itemVar, $item );
		...
		if( Gen::GetArrField( $aPrm, 'zoom' ) == 1 )
		{

			$bZoom = !( Gen::GetArrField( $aPrm, 'is_mobile' ) == 1 && Gen::GetArrField( $aPrm, 'mobile_zoom' ) == 'false' );
			foreach( HtmlNd::ChildrenAsArr( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," wpgs-for ")]//img', $item ) ) as $itemSld )
			{
				$itemSldWrap = HtmlNd::CreateTag( $doc, 'div', array( 'class' => array( $bZoom ? 'zoomtoo-container' : null ), 'data-zoom-image' => $itemSld -> getAttribute( 'data-large_image' ) ) );
				$itemSld -> parentNode -> insertBefore( $itemSldWrap, $itemSld );
				$itemSldWrap -> appendChild( $itemSld );
			}
		}

		$a[] = array(
			'enable' => true,
			'selCss' => '.woocommerce-product-gallery.woo-product-gallery-slider .wpgs-for',
			'bps' => array(
				0 => array(
					'adaptiveHeight' => Gen::GetArrField( $aPrm, 'slider_adaptiveHeight' ) != 'false',
					'infinite' => Gen::GetArrField( $aPrm, 'slider_infinity' ) != 'false',
					'dots' => Gen::GetArrField( $aPrm, 'slider_dots' ) != 'false',
					'dotsClass' => array( 'slick-dots', 'wpgs-dots' ),
					'arrows' => Gen::GetArrField( $aPrm, 'slider_nav' ) == 'true',
		...
		$a[] = array(
			'enable' => true,
			'selCss' => '.woocommerce-product-gallery.woo-product-gallery-slider .wpgs-nav',
			'alignItems' => 'start',
			'bps' => array(
				0 => array(
					'vertical' => Gen::GetArrField( $aPrm, 'thumb_v' ) != 'bottom',
					'slidesToShow' => ( int )Gen::GetArrField( $aPrm, array( 'thumb_to_show' ) ),
					'slidesToScroll' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_mobile_thumb_scroll_by' ),
					'infinite' => Gen::GetArrField( $aPrm, 'slider_infinity' ) != 'false',
					'centerMode' => Gen::GetArrField( $aPrm, 'centerMode' ) == 1,
					'arrows' => Gen::GetArrField( $aPrm, 'thumbnails_nav' ) == 1,
				),

				767 => array(
					'vertical' => Gen::GetArrField( $aPrm, 'thumb_position_mobile' ) != 'bottom',
					'slidesToShow' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_mobile_thumb_to_show' ),
					'slidesToScroll' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_mobile_thumb_scroll_by' ),
					'infinite' => Gen::GetArrField( $aPrm, 'slider_infinity' ) != 'false',
					'centerMode' => Gen::GetArrField( $aPrm, 'centerMode' ) == 1,
					'arrows' => Gen::GetArrField( $aPrm, 'thumbnails_nav' ) == 1,
				...
				1025 => array(
					'vertical' => Gen::GetArrField( $aPrm, 'thumb_v_tablet' ) != 'bottom',
					'slidesToShow' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_tabs_thumb_to_show' ),
					'slidesToScroll' => ( int )Gen::GetArrField( $aPrm, 'thumbnails_tabs_thumb_scroll_by' ),
					'infinite' => Gen::GetArrField( $aPrm, 'slider_infinity' ) != 'false',
					'centerMode' => Gen::GetArrField( $aPrm, 'centerMode' ) == 1,
					'arrows' => Gen::GetArrField( $aPrm, 'thumbnails_nav' ) == 1,
				),
...
function _ProcessCont_Cp_wooPrdGallSld_GetCmnPrms( &$ctxProcess, $xpath )
{
	$cfg = array();

	$contScr = ( string )_Cp_GetScrCont( $ctxProcess, $xpath, './/script[@id="wpgs-public-js-extra"][contains(text(),"wpgs_js_data")]', null );
	if( !$contScr )
		return( $cfg );

	$posStart = array();
	if( !preg_match( '@var\\s+wpgs_js_data\\s*=\\s*{@', $contScr, $posStart, PREG_OFFSET_CAPTURE ) )
		return( $cfg );
	...
	$cfg = @json_decode( Gen::JsObjDecl2Json( substr( $contScr, $posStart, $pos - $posStart ) ), true );
	return( $cfg );
}
...
function _ProcessCont_Cp_wooGsPrdGal( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	...
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," wcgs-woocommerce-product-gallery ")]' ) as $item )
	{
		...
		if( Gen::GetArrField( $aPrm, array( 'wcgs_settings', 'gallery_layout_on_mobile' ) ) == '1' )
			continue;
		...
		if( Gen::GetArrField( $aPrm, array( 'wcgs_settings', 'gallery_layout' ) ) != 'hide_thumb' && $itemThumbnails = HtmlNd::FirstOfChildren( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," gallery-navigation-carousel ")][contains(concat(" ",normalize-space(@class)," ")," spswiper ")]', $item ) ) )
		{
			...
			if( count( $aThumbs ) > 1 )
			{
				...
				$aPrmSwpr = array( 'cssIdSelPrefix' => '.gallery-navigation-carousel', 'cssClassInited' => 'spswiper-initialized', 'cssSelSlide' => '.spswiper-slide', 'cssClass' => 'spswiper', 'cssClassWrapper' => 'spswiper-wrapper' );
				if( $isMultiRowThumbsLayout )
					$aPrmSwpr = array_merge( $aPrmSwpr, array( 'breakpoints' => array( array( 'slidesPerView' => 1 ) ), 'breakpointsSpace' => array( array( 'space' => 0 ) ) ) );
				else
					$aPrmSwpr = array_merge( $aPrmSwpr, array( 'breakpoints' => array( array( 'slidesPerView' => $thumbnailsItemToShow ) ), 'breakpointsSpace' => array( array( 'space' => $thumbnails_sliders_space ) ), 'isVert' => $galleryLayout == 'vertical' ) );
				$contCmnStyle .= _ProcessCont_Cp_swiper_AdjustItem( $itemThumbnails, $itemThumbnails, $aPrmSwpr, $ctx, $ctxProcess, $doc, $xpath );
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
...

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "function seraph_accel_cp_wooGsPrdGal_calcSizes(g){function h(a){var b=a.offsetWidth;a=getComputedStyle(a);return b-parseInt(a.getPropertyValue(\"padding-left\"),10)-parseInt(a.getPropertyValue(\"padding-right\"),10)}function n(a,b,c,d,e){if(1>=c)a.eNavWrp.style.setProperty(\"display\",\"none\");else{var l=wcgs_object.wcgs_settings.thumbnails_sliders_space&&wcgs_object.wcgs_settings.thumbnails_sliders_space.height?wcgs_object.wcgs_settings.thumbnails_sliders_space.height:6;a.eCrsl.style.setProperty(\"max-width\",\n\"\"+(b-d-l)+\"px\");a.eCrsl.style.setProperty(\"width\",\"\"+(b-d-l)+\"px\");a.eNavWrp.style.setProperty(\"width\",\"\"+d+\"px\");var m=q(a);a.eNavWrp.style.setProperty(\"max-height\",\"\"+m+\"px\");wcgs_object.wcgs_settings.image_crop_size&&\"Hard-crop\"!==wcgs_object.wcgs_settings.image_crop_size.unit&&a.eCrsl.classList.add(\"vertically-center\");a.eCrsl.style.setProperty(\"height\",\"\"+m+\"px\");a.eCrsl.querySelectorAll(\".spswiper-slide\").forEach(function(p){p.style.setProperty(\"height\",\"\"+m+\"px\");p.style.setProperty(\"width\",\n\"\"+(b-d-l)+\"px\")});a.eNav.style.setProperty(\"--lzl-swpr-sz\",\"\"+a.eNav.offsetHeight+\"px\")}}function q(a){var b=0;if(wcgs_object.wcgs_settings.slider_height_type&&\"fix_height\"==wcgs_object.wcgs_settings.slider_height_type)return r(a);a.eCrsl.querySelectorAll(\"img\").forEach(function(c){var d=c.getAttribute(\"width\"),e=c.getAttribute(\"height\");d&&e?(d=parseInt(d,10),e=parseInt(e,10),d&&e&&(c=e/d*c.offsetWidth,c>b&&(b=c))):c.offsetHeight>b&&(b=c.offsetHeight)});return b}function r(a){const b=wcgs_object.wcgs_settings.slider_height||\n{};return 520>a.windowWidth&&b.height2?b.height2||500:736>a.windowWidth&&b.height?b.height||500:b.width||500}var f={e:g,windowWidth:window.innerWidth};f.eCrsl=g.querySelector(\".wcgs-carousel\");f.eNavWrp=g.querySelector(\".gallery-navigation-carousel-wrapper\");f.eNav=f.eNavWrp.querySelector(\".gallery-navigation-carousel\");var k=768>f.windowWidth;!wcgs_object.wcgs_settings.gallery_layout_on_mobile||\"1\"!=wcgs_object.wcgs_settings.gallery_layout_on_mobile||\"vertical\"!=wcgs_object.wcgs_settings.gallery_layout&&\n\"vertical_right\"!=wcgs_object.wcgs_settings.gallery_layout?k&&wcgs_object.wcgs_settings.slider_layout_on_mobile&&\"1\"==wcgs_object.wcgs_settings.slider_layout_on_mobile&&(wcgs_object.wcgs_settings.gallery_layout=\"hide_thumb\",f.e.classList.remove(\"grid\",\"wcgs_vertical_scroll_nav\",\"vertical\"),f.eCrsl.classList.remove(\"vertical\"),f.eCrsl.classList.add(\"wcgs-mobile-layout\",\"horizontal\")):k&&(wcgs_object.wcgs_settings.gallery_layout=\"horizontal\",f.eCrsl.classList.remove(\"vertical\"),f.eCrsl.classList.add(\"horizontal\"),\nf.eNav.classList.remove(\"vertical\"),f.eNav.classList.add(\"horizontal\"),f.e.classList.remove(\"vertical\"),f.e.classList.add(\"horizontal\"));k=function(a){var b=document.querySelector(\"body\"),c=wcgs_object.wcgs_settings.gallery_width,d=h(document.querySelector(wcgs_object.wcgs_product_wrapper));var e=h(a.e.parentNode);e=e>d?d:e;d=\"%\";b.classList.contains(\"theme-flatsome\")&&(e=h(document.querySelector(\".single-product .product .row.content-row\")));if(\"vertical\"==wcgs_object.wcgs_settings.gallery_layout||\n\"vertical_right\"==wcgs_object.wcgs_settings.gallery_layout||\"hide_thumb\"==wcgs_object.wcgs_settings.gallery_layout||100<c)c=c*e/100,100<wcgs_object.wcgs_settings.gallery_width&&(c=wcgs_object.wcgs_settings.gallery_width);a.e.parentNode.querySelectorAll(\".hestia-product-image-wrap\").length&&(e=document.querySelector(\".wcgs-wcgs-woocommerce-product-gallery\"))&&e.parentNode&&(e=e.parentNode.querySelector(\".hestia-product-image-wrap\"))&&(c=h(e1));(b.classList.contains(\"et_divi_builder\")||b.classList.contains(\"theme-Divi\"))&&\n(e=document.querySelector(\".et-db #et-boc .et-l .et_pb_row .et_pb_column:has(#wpgs-gallery)\"))&&(c=h(e));document.querySelectorAll(\"#wpgs-gallery ~ .summary\").length||(c=a.e.parentNode.getBoundingClientRect().width);992>a.windowWidth&&0<wcgs_object.wcgs_settings.gallery_responsive_width.width&&(c=wcgs_object.wcgs_settings.gallery_responsive_width.width,d=wcgs_object.wcgs_settings.gallery_responsive_width.unit);768>a.windowWidth&&(c=wcgs_object.wcgs_settings.gallery_responsive_width.height,d=wcgs_object.wcgs_settings.gallery_responsive_width.unit);\n480>a.windowWidth&&(c=wcgs_object.wcgs_settings.gallery_responsive_width.height2,d=wcgs_object.wcgs_settings.gallery_responsive_width.unit);\"em\"==d&&(a=c,b=document.documentElement.style.fontSize||window.getComputedStyle(document.documentElement).fontSize||\"16px\",c=a*parseFloat(b));return c}(f);(function(a,b){var c=\"%\";if(\"vertical\"==wcgs_object.wcgs_settings.gallery_layout||\"vertical_right\"==wcgs_object.wcgs_settings.gallery_layout||\"hide_thumb\"==wcgs_object.wcgs_settings.gallery_layout||100<b){c=\n\"px\";var d=a.e.parentNode.getBoundingClientRect().width;b=d>b?b:d;a.e.style.setProperty(\"max-width\",\"100%\")}a.e.style.setProperty(\"min-width\",\"auto\");a.e.style.setProperty(\"width\",\"\"+b+c);d=c;c=a.e.parentNode;if(\"%\"==d)a=100-b,a=20<a?\"calc(\"+a+\"% - 50px)\":\"\";else a:{var e=h(c);if(e>h(a.e)+100&&(a=e-b,150<a)){a=a-50+d;break a}a=\"\"}(c=c.querySelector(\".summary\"))&&c.style.setProperty(\"max-width\",a)})(f,k);g=g.parentNode.getBoundingClientRect().width;(function(a,b){const c=a.eNav.querySelectorAll(\".wcgs-thumb\").length,\nd=b/100*(wcgs_object.wcgs_settings.vertical_thumbs_width||20),e=wcgs_object.wcgs_settings.thumbnails_sliders_space&&wcgs_object.wcgs_settings.thumbnails_sliders_space.width?wcgs_object.wcgs_settings.thumbnails_sliders_space.width:6;\"horizontal\"!=wcgs_object.wcgs_settings.gallery_layout&&\"horizontal_top\"!=wcgs_object.wcgs_settings.gallery_layout&&\"multi_row_thumb\"!=wcgs_object.wcgs_settings.gallery_layout&&(\"hide_thumb\"==wcgs_object.wcgs_settings.gallery_layout?(a.eCrsl.style.setProperty(\"width\",\"\"+\nb+\"px\"),a.eNavWrp.style.setProperty(\"display\",\"none\")):n(a,b,c,d,e))})(f,g>k?k:g)}(function(g){function h(n){g.querySelectorAll(\".wcgs-woocommerce-product-gallery\").forEach(seraph_accel_cp_wooGsPrdGal_calcSizes)}g.addEventListener(\"seraph_accel_calcSizes\",h,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){g.removeEventListener(\"seraph_accel_calcSizes\",h,{capture:!0,passive:!0})})})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
...

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, str_replace( array( '_E_FADE_' ), array( Gen::GetArrField( $aPrm, array( 'flexslider', 'animation' ), 'fade' ) == 'fade' ? '1' : '0' ), "(function(a){function c(d){d.target.querySelectorAll(\".woocommerce-product-gallery__wrapper-js-lzl-ing,.flex-viewport-js-lzl-ing,.flex-control-thumbs-js-lzl-ing\").forEach(function(b){b.parentNode.removeChild(b)})}seraph_accel_izrbpb.add(function(){a.addEventListener(\"DOMContentLoaded\",function(){if(window.jQuery)jQuery(a).on(\"wc-product-gallery-after-init\",c)},{capture:!0,passive:!0})})})(document)" ) );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
		...
		if( ($aPrm[ '_bSmoothHeight' ]??null) )
		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, str_replace( array( '_E_FADE_' ), array( Gen::GetArrField( $aPrm, array( 'flexslider', 'animation' ), 'fade' ) == 'fade' ? '1' : '0' ), "function seraph_accel_cp_wooPrdGall_calcSizes(a){var c=a;if(!_E_FADE_&&((c=a.querySelector(\".flex-viewport\"))||(c=a.querySelector(\".flex-viewport-js-lzl-ing\")),!c))return;var b=a.querySelector(\".flex-viewport > :first-child > :first-child > img\");b||(b=a.querySelector(\".flex-viewport-js-lzl-ing > :first-child > :first-child img\"));b||(b=a.querySelector(\".woocommerce-product-gallery__wrapper > :first-child img\"));b&&(b=b.getBoundingClientRect().height,a.style.setProperty(\"--lzl-vp-h\",\"\"+b+\"px\"),c.style.setProperty(\"height\",\n\"var(--lzl-vp-h)\"))}(function(a){function c(b){a.querySelectorAll(\".woocommerce-product-gallery.woocommerce-product-gallery--with-images\").forEach(seraph_accel_cp_wooPrdGall_calcSizes)}a.addEventListener(\"seraph_accel_calcSizes\",c,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){a.removeEventListener(\"seraph_accel_calcSizes\",c,{capture:!0,passive:!0})})})(document)" ) );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "function seraph_accel_cp_wooPrdGallAstrThmbsHeight_calcSizes(b){var a=b.querySelector(\".flex-viewport\");a||(a=b.querySelector(\".flex-viewport-js-lzl-ing\"));a&&(a=a.getBoundingClientRect().height,b.style.setProperty(\"--lzl-ast-fcn-h\",\"\"+a+\"px\"))}\n(function(b){b.addEventListener(\"seraph_accel_calcSizes\",function(a){b.querySelectorAll(\".ast-product-gallery-layout-vertical .woocommerce-product-gallery.woocommerce-product-gallery--with-images\").forEach(seraph_accel_cp_wooPrdGallAstrThmbsHeight_calcSizes)},{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){})})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemScript -> setAttribute( 'type', 'text/javascript' );
			$itemScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemScript, "(function(a){function c(d){d.target.querySelectorAll(\".swiper.js-lzl-ing\").forEach(function(b){b.parentNode.removeChild(b)})}seraph_accel_izrbpb.add(function(){a.addEventListener(\"DOMContentLoaded\",function(){if(window.jQuery)jQuery(a).on(\"wc-product-gallery-after-init\",c)},{capture:!0,passive:!0})})})(document)" );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemScript, $ctxProcess[ 'ndBody' ] -> firstChild );
		}
	}
}

function _ProcessCont_Cp_wooNasaPrdGall( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$a = array();

	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," nasa-single-product-slide ")]' ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;
		...
		$a[] = array(
			'enable' => true,
			'selCss' => '.nasa-single-product-slide .nasa-single-product-main-image',
			'bps' => array(
				0 => array(
					'slidesToShow' => ( int )$item -> getAttribute( 'data-num_main' ),
					'infinite' => $item -> getAttribute( 'data-infinite' ) != 'false',
					'dots' => $item -> getAttribute( 'data-dots' ) != 'false',
				),
			),
		);
		...
		$a[] = array(
			'enable' => true,
			'selCss' => '.nasa-single-product-slide .product-thumbnails',
			'bps' => array(
				0 => array(
					'slidesToShow' => ( int )$item -> getAttribute( 'data-num_thumb' ),
					'vertical' => true,
				),
			),
		);
...
function _ProcessCont_Cp_xstrThSwpr( $ctx, &$ctxProcess, $settFrm, $settCache, $settImg, $settCdn, $doc, $xpath )
{
	...
	foreach( $xpath -> query( './/body[contains(concat(" ",normalize-space(@class)," ")," wp-theme-xstore ")]//*[contains(concat(" ",normalize-space(@class)," ")," swiper-container ")]' ) as $item )
	{
		...
		$aPrm[ 'cssIdSelPrefix' ] = $selId;
		HtmlNd::AddRemoveAttrClass( $item, array( 'lzl-js', 'scnd-ed-lzl' ) );

		$nSpace = 0;
		if( $item -> hasAttribute( 'data-space' ) )
			$nSpace = ( int )$item -> getAttribute( 'data-space' );
		...
		if( !$bVer )
		{
			foreach( array( array( 481 - 1, 'data-xs-slides' ), array( 1199 - 1, 'data-sm-slides' ), array( null, 'data-lt-slides' ) ) as $m )
				if( $item -> hasAttribute( $m[ 1 ] ) )
					$aPrm[ 'breakpoints' ][] = array( 'maxWidth' => $m[ 0 ], 'slidesPerView' => ( int )$item -> getAttribute( $m[ 1 ] ) );
		}
		else if( $item -> hasAttribute( 'data-breakpoints' ) )
		{
			foreach( array( array( 640 - 1, 'data-xs-slides' ), array( 1024 - 1, 'data-sm-slides' ), array( 1370 - 1, 'data-md-slides' ), array( null, 'data-lt-slides' ) ) as $m )
				if( $item -> hasAttribute( $m[ 1 ] ) )
					$aPrm[ 'breakpoints' ][] = array( 'maxWidth' => $m[ 0 ], 'slidesPerView' => ( int )$item -> getAttribute( $m[ 1 ] ) );
		}

		$aPrm[ 'breakpointsSpace' ] = array( array( 'space' => $nSpace ) );

		$contCmnStyle .= _ProcessCont_Cp_swiper_AdjustItem( $item, $item, $aPrm, $ctx, $ctxProcess, $doc, $xpath );
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		$ctxProcess[ 'aCssRpl' ][ '@(\\.second-initialized)@' ] = ':is(.second-initialized,.scnd-ed-lzl)';
...
function _ProcessCont_Cp_btCntSld( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$a = array();
	$idNext = 0;
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," bt_bb_content_slider ")]/*[contains(concat(" ",normalize-space(@class)," ")," slick-slider ")][@data-slick]' ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;

		$dataSett = @json_decode( $item -> getAttribute( 'data-slick' ), true );
		if( !$dataSett )
			continue;

		$classid = 'lzl-id-' . $idNext++;
		HtmlNd::AddRemoveAttrClass( $item -> parentNode, array( $classid ) );

		$a[] = array(
			'enable' => true,
			'selCss' => '.' . $classid . '.bt_bb_content_slider .slick-slider',
			'bps' => array(
				0 => array(
					'dots' => ($dataSett[ 'dots' ]??null),
					'arrows' => ($dataSett[ 'arrows' ]??null),
...
function _ProcessCont_Cp_btMenu( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	if( !HtmlNd::FirstOfChildren( $xpath -> query( './/body[contains(concat(" ",normalize-space(@class)," ")," btMenuLeftEnabled ")]' ) ) )
		return;

	HtmlNd::AddRemoveAttrClass( $ctxProcess[ 'ndBody' ], array( 'btMenuInitFinished' ) );

	if( ( $ctxProcess[ 'mode' ] & 1 ) )
	{
		$ctxProcess[ 'aCssCrit' ][ '@\\.btMenuHorizontal@' ] = true;
		$ctxProcess[ 'aCssCrit' ][ '@\\.btMenuVertical@' ] = true;

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemCmnScript -> setAttribute( 'type', 'text/javascript' );
			$itemCmnScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemCmnScript, 'function seraph_accel_cp_btMenu_calcSizes(){var b=document,d=window,a=b.body.classList,c,f=a.contains("rtl")?"Right":"Left",g=a.contains("btBelowMenu");a.contains("btStickyEnabled");var h=a.contains("btMenuVerticalLeftEnabled")||a.contains("btMenuVerticalRightEnabled"),k=a.contains("btMenuFullScreenCenter");1200>d.innerWidth?(a.add("btMenuVertical","btMenuVertical"+f),a.remove("btMenuHorizontal")):(a.remove("btMenuVertical","btMenuVertical"+f,"btMenuVerticalOn"),a.add("btMenuHorizontal"));if(!g){if(1200>
d.innerWidth||h||k){if(c=b.querySelector(".btVerticalHeaderTop, .bt-vertical-header-top"))var e=c.getBoundingClientRect().height}else!a.contains("btStickyHeaderActive")&&(c=b.querySelector(".mainHeader"))&&(e=c.getBoundingClientRect().height);void 0!==e&&b.querySelectorAll(".btContentWrap, .bt-content-wrap").forEach(function(l){l.style.setProperty("padding-top",e+"px")})}}seraph_accel_cp_btMenu_calcSizes();
(function(b,d){b.addEventListener("seraph_accel_calcSizes",seraph_accel_cp_btMenu_calcSizes,{capture:!0,passive:!0});seraph_accel_izrbpb.add(function(){b.removeEventListener("seraph_accel_calcSizes",seraph_accel_cp_btMenu_calcSizes,{capture:!0,passive:!0})})})(document,window);
' );
			$ctxProcess[ 'ndBody' ] -> insertBefore( $itemCmnScript, $ctxProcess[ 'ndBody' ] -> firstChild );

			if( $itemContentWrap = HtmlNd::FirstOfChildren( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," btContentWrap ")]|.//*[contains(concat(" ",normalize-space(@class)," ")," bt-content-wrap ")]' ) ) )
			{
				$itemCmnScript = $doc -> createElement( 'script' );
				if( $ctxProcess[ 'bJsCssAddType' ] )
					$itemCmnScript -> setAttribute( 'type', 'text/javascript' );
				$itemCmnScript -> setAttribute( 'seraph-accel-crit', '1' );
				HtmlNd::SetValFromContent( $itemCmnScript, 'seraph_accel_cp_btMenu_calcSizes()' );

				$itemContentWrap -> insertBefore( $itemCmnScript, $itemContentWrap -> firstChild );
			}
		}
	}
}

function _ProcessCont_Cp_bdeSwpr( $ctx, &$ctxProcess, $settFrm, $settCache, $settImg, $settCdn, $doc, $xpath )
{
	$contCmnStyle = '';

	$adjusted = false;
	$cfgCmn = null;
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," breakdance-swiper-wrapper ")]' ) as $item )
	{
		...
		$itemSwpr = HtmlNd::FirstOfChildren( $xpath -> query( './*[contains(concat(" ",normalize-space(@class)," ")," breakdance-swiper-wrapper ")]/*[contains(concat(" ",normalize-space(@class)," ")," swiper ")]', $item ) );
		if( !$itemSwpr )
			continue;

		$id = null;
		foreach( HtmlNd::GetAttrClass( $item ) as $cl )
		{
			if( !preg_match( '@^bde-[\\w\\-]+-[\\d]+@', $cl ) )
				continue;
		...
		if( !$cfgCmn )
		{
			$cfgCmn = _ProcessCont_Cp_bdeSwpr_GetCmnPrms( $ctxProcess, $xpath );
			if( !$cfgCmn )
				return;
		}

		$cfg = _ProcessCont_Cp_bdeSwpr_GetPrms( $ctxProcess, $xpath, $id );
		if( !$cfg )
			continue;
		...
		$aPrm = array(
			...
			'cssClassInited' => 'swiper-initialized',
			'cssIdSelPrefix' => '.' . $id . ' ',
			'paginationType' => Gen::GetArrField( $cfg, array( 'paginationSettings', 'type' ) ),
		);

		HtmlNd::AddRemoveAttrClass( $itemSwpr, array( 'lzl-js', 'swpr-ed-lzl' ) );

		if( !isset( $cfgCmn[ 'breakpoints' ] ) )
			continue;

		foreach( $cfgCmn[ 'breakpoints' ] as $bpId => $bp )
		{
			if( $v = ($cfg[ 'settings' ][ 'advanced' ][ 'slides_per_view' ][ $bpId ]??null) )
				$aPrm[ 'breakpoints' ][] = array( 'minWidth' => ($bp[ 'minWidth' ]??null), 'maxWidth' => ($bp[ 'maxWidth' ]??null), 'slidesPerView' => $v );

			if( $a = ($cfg[ 'settings' ][ 'advanced' ][ 'between_slides' ][ $bpId ]??null) )
				$aPrm[ 'breakpointsSpace' ][] = array( 'minWidth' => ($bp[ 'minWidth' ]??null), 'maxWidth' => ($bp[ 'maxWidth' ]??null), 'space' => ($a[ 'number' ]??null) );
		}

		$contCmnStyle .= _ProcessCont_Cp_swiper_AdjustItem( $item, $itemSwpr, $aPrm, $ctx, $ctxProcess, $doc, $xpath );
...
function _ProcessCont_Cp_bdeSwpr_GetCmnPrms( &$ctxProcess, $xpath )
{
	$cfg = array();

	$contScr = ( string )_Cp_GetScrCont( $ctxProcess, $xpath, './/script[contains(text(),"window.BreakdanceFrontend.data")]', null );
	if( !$contScr )
		return( $cfg );

	$posStart = array();
	if( !preg_match( '@window\\s*\\.\\s*BreakdanceFrontend\\s*\\.\\s*data\\s*=\\s*{@', $contScr, $posStart, PREG_OFFSET_CAPTURE ) )
		return( $cfg );
	...
	if( isset( $cfg[ 'breakpoints' ] ) )
	{
		$aBp = array();
		foreach( $cfg[ 'breakpoints' ] as $bp )
			$aBp[ $bp[ 'id' ] ] = $bp;
		uasort( $aBp,
			function( $a1, $a2 )
			{
				return( -1 * Gen::VarCmp( ($a1[ 'maxWidth' ]??99999999), ($a2[ 'maxWidth' ]??99999999) ) );
			}
		);
		$cfg[ 'breakpoints' ] = $aBp;
	}
...
function _ProcessCont_Cp_bdeSwpr_GetPrms( &$ctxProcess, $xpath, $id )
{
	$cfg = array();

	$contScr = ( string )_Cp_GetScrCont( $ctxProcess, $xpath, './/script[contains(text(),"\'' . $id . '\'")]', null );
	if( !$contScr )
		return( $cfg );

	$posStart = array();
	if( !preg_match( '@window\\s*\\.BreakdanceSwiper\\s*\\(\\s*\\)\\s*\\.update\\s*\\(\\s*{@', $contScr, $posStart, PREG_OFFSET_CAPTURE ) )
		return( $cfg );
...
function _ProcessCont_Cp_bdeTabs( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$adjusted = false;

	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," bde-tabs ")]' ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;

		if( $itemTabTitleFirst = HtmlNd::FirstOfChildren( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," bde-tabs__tabslist ")]/*[1]', $item ) ) )
			HtmlNd::AddRemoveAttrClass( $itemTabTitleFirst, array( 'is-active' ) );

		if( $itemTabBodyFirst = HtmlNd::FirstOfChildren( $xpath -> query( '(.//*[contains(concat(" ",normalize-space(@class)," ")," bde-tabs__panel ")])[1]', $item ) ) )
			HtmlNd::AddRemoveAttrClass( $itemTabBodyFirst, array( 'is-active' ) );
...
function _ProcessCont_Cp_tpgbAni( $ctx, &$ctxProcess, $settFrm, $doc, $xpath )
{
	$adjusted = false;
	foreach( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," tpgb-view-animation ")][@data-animationsetting]' ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;

		$dataSett = ( array )@json_decode( $item -> getAttribute( 'data-animationsetting' ), true );

		$bAdjusted = false;
		foreach( array( 'xs', 'sm', 'md' ) as $dim )
		{
			$sAniName = Gen::GetArrField( $dataSett, array( 'anime', $dim ), '' );
			if( !$sAniName )
				continue;

			$ctxProcess[ 'aCssCrit' ][ '@\\.tpgb_' . $sAniName . '@' ] = true;
			$bAdjusted = true;
		}

		if( $bAdjusted )
		{
			HtmlNd::AddRemoveAttrClass( $item, array( 'an-lzl' ) );
			$adjusted = true;
		}
	}
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		$ctxProcess[ 'aCssCrit' ][ '@\\.tpgb_animated@' ] = true;

		{

			$ctx -> aAniAppear[ '.tpgb-view-animation.an-lzl:not(.tpgb_animated)' ] = "function(b){var c,d=\"md\",e=window.innerWidth;768>e?d=\"xs\":1025>e&&(d=\"sm\");try{var a=JSON.parse(b.getAttribute(\"data-animationsetting\"))}catch(f){}a||(a={});a.anime&&(c=a.anime[d]);c&&b.classList.add(\"tpgb_\"+c);b.classList.add(\"tpgb_animated\")}";
		}

		{
			...
			if( $ctxProcess[ 'bJsCssAddType' ] )
				$itemCmnScript -> setAttribute( 'type', 'text/javascript' );
			$itemCmnScript -> setAttribute( 'seraph-accel-crit', '1' );
			HtmlNd::SetValFromContent( $itemCmnScript, '(function(f,g){seraph_accel_izrbpb.add(function(){f.querySelectorAll(".tpgb-view-animation.an-lzl.tpgb_animated").forEach(function(b){try{var a=JSON.parse(b.getAttribute("data-animationsetting"))}catch(h){}a||(a={});if(a.anime){var c=["md","xs","sm"],d,e;for(e in c)(d=a.anime[c[e]])&&b.classList.remove("tpgb_"+d)}})})})(document,window);
' );
...
function _ProcessCont_Cp_vhcThCarFldGall( $ctx, &$ctxProcess, $settFrm, $settCache, $settImg, $settCdn, $doc, $xpath )
{
	$adjusted = false;
	foreach( $xpath -> query( './/vehica-car-field-gallery' ) as $item )
	{
		if( FramesCp_CheckExcl( $ctxProcess, $doc, $settFrm, $item ) || !ContentProcess_IsItemInFragments( $ctxProcess, $item ) )
			continue;

		$itemCont = HtmlNd::FirstOfChildren( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," vehica-swiper-container ")]', $item ) );
		if( !$itemCont )
			continue;

		$itemTpls = HtmlNd::FirstOfChildren( $xpath -> query( './/template', $itemCont ) );
		if( !$itemTpls )
			continue;

		$itemCount = HtmlNd::FirstOfChildren( $xpath -> query( './/*[contains(concat(" ",normalize-space(@class)," ")," vehica-car-gallery__count ")]', $itemTpls ) );
		if( !$itemCount )
			continue;

		$itemCount = $itemCount -> cloneNode( true );
		for( $itemC = null; $itemC = HtmlNd::GetNextTreeChild( $itemCount, $itemC );  )
			if( is_string( $itemC -> nodeValue ) )
				$itemC -> nodeValue = preg_replace( '@{\\s*{\\s*galleryProps\\s*\\.\\s*activeIndex\\s*\\+\\s*1\\s*}\\s*}@', '1', $itemC -> nodeValue );
		$itemCont -> insertBefore( $itemCount, $itemTpls );

		HtmlNd::AddRemoveAttrClass( $itemCount, array( 'js-lzl-ing' ) );
	...
	if( ( $ctxProcess[ 'mode' ] & 1 ) && $adjusted )
	{
		{
			$contCmnStyle = '';
			$contCmnStyle .= '.vehica-swiper-container:has(.vehica-car-gallery__count:not(.js-lzl-ing)) .vehica-car-gallery__count.js-lzl-ing { display: none; }';
