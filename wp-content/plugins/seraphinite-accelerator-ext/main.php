...
function OnActivate( $bAllMultisites )
{
	CacheInitEnv( Plugin::SettGetGlobal(), Plugin::SettGet(), true, $bAllMultisites );
	CacheInitQueueTable( true );
}

function OnDeactivate( $bAllMultisites )
{
	CacheInitEnv( null, null, false, $bAllMultisites );
}
...
{
	add_menu_page( Plugin::GetPluginString( 'TitleLong' ), Plugin::GetNavMenuTitle(), 'manage_options', 'seraph_accel_manage',																		$accepted ? 'seraph_accel\\_ManagePage' : 'seraph_accel\\Plugin::OutputNotAcceptedPageContent', Plugin::FileUri( 'icon.png?v=2.29.11', __FILE__ ) );
...
function _OnAdminNotices()
{
	...
	if( Gen::GetArrField( $sett, array( 'contPr', 'enable' ), false ) && PluginLic::CheckFeature() != Gen::S_OK )
	{
...
function _InitCatchDataUpdate( $level )
{
	...
	if( Gen::GetArrField( $sett, array( 'cache', 'updGlobs', 'tpl', 'enable' ) ) )
	{
		if( _CheckUpdatePost_Rtn::$level < 2 && $level >= 2 )
			add_action( 'post_updated', 'seraph_accel\\_OnTplUpdate', 99999, 3 );
	...
	if( Gen::GetArrField( $sett, array( 'cache', 'updGlobs', 'flntFrm', 'enable' ) ) )
	{
		if( _CheckUpdatePost_Rtn::$level < 2 && $level >= 2 )
		{
			add_action( 'fluentform/before_updating_form', 'seraph_accel\\_OnFlntFrmUpdate', 99999, 0 );
			add_action( 'fluentform/after_save_form_settings', 'seraph_accel\\_OnFlntFrmUpdate', 99999, 0 );
		}
...
function OnPreInit( $isAdminMode )
{
...
function OnInit( $isAdminMode )
{
	...
	if( ($settContPr[ 'enable' ]??null) )
		add_action( 'wp_loaded',
			function()
			{
				$ctxAlwsRpl = new AnyObj();

				$sett = Plugin::SettGet();
				$ctxAlwsRpl -> settContPr = Gen::GetArrField( $sett, array( 'contPr' ), array() );

				$ctxAlwsRpl -> onContApply =
					function( $ctxAlwsRpl, $buffer )
					{
						return( ContentProcess_AlwaysReplacements( $ctxAlwsRpl -> a, $ctxAlwsRpl -> settContPr, $buffer ) );
					};

				if( $ctxAlwsRpl -> a = ContentProcess_GetAlwaysReplacements( $sett, $ctxAlwsRpl -> settContPr ) )
					ob_start( array( $ctxAlwsRpl, 'onContApply' ) );
			}
		);
	...
	if( $isGet && $cacheEnable && isset( $_REQUEST[ 'seraph_accel_gbnr' ] ) )
	{
		add_action( 'init',
			function()
			{
...
function _OnPostMetaUpdated( $postId, $metaKey, $metaValue )
{
	...
	foreach( Gen::GetArrField( $sett, array( 'cache', 'updPostMetaExcl' ), array() ) as $exclPattern )
	{
		...
		if( ContProcGetExclStatus_KeyValMatch( $exclPattern, ( string )$metaKey, '', '' ) )
		{
			if( $tmSch !== null )
...
function _OnTplUpdate( $postId, $post, $postBefore )
{
	...
	global $seraph_accel_g_globUpdated;
	$sett = Plugin::SettGet();
	$seraph_accel_g_globUpdated[ 'tpl' ] = Gen::GetArrField( $sett, array( 'cache', 'updGlobs', 'op' ), 0 );
}
...
function _OnFlntFrmUpdate()
{
	global $seraph_accel_g_globUpdated;
	$sett = Plugin::SettGet();
	$seraph_accel_g_globUpdated[ 'flntFrm' ] = Gen::GetArrField( $sett, array( 'cache', 'updGlobs', 'op' ), 0 );
}
			...
			function( $callbacks_args, $box )
			{
				...
				echo( Ui::TagOpen( 'div', array( 'class' => 'blck' ) ) );
				{
					...
					echo( Ui::SettBlock_Begin( array( 'class' => 'compact' ) ) );
					{
						...
						if( Gen::GetArrField( Plugin::SettGet(), array( 'contPr', 'enable' ), false )&& PluginLic::CheckFeature() != Gen::S_OK )
						{
						...
						echo( Ui::SettBlock_Item_Begin( esc_html_x( 'CleanupLbl', 'admin.Manage_Status', 'seraphinite-accelerator' ), array( 'class' => array( 'blck', 'cleanup' ) ) ) );
						{
							...
							echo( Ui::TagOpen( 'p' ) );
							{
								echo( Ui::Button( Wp::GetLocString( 'Start', null, 'seraphinite-accelerator' ), false, null, null, 'button', array( 'class' => array( 'ctlSpaceAfter', 'ctlVaMiddle' ), 'style' => array( 'min-width' => '7em' ), 'onclick' => 'seraph_accel.Manager._int.OnCacheOp(this,1,"' . wp_create_nonce( 'op-1' ) . '"); return false;' ) ) );
								echo( Ui::Button( Wp::GetLocString( 'Cancel' ), false, null, null, 'button', array( 'class' => array( 'ctlSpaceAfter', 'ctlVaMiddle', 'cancel' ), 'style' => array( 'min-width' => '7em' ), 'disabled' => true, 'onclick' => 'seraph_accel.Manager._int.OnCacheOpCancel(this,1,"' . wp_create_nonce( 'op-cancel' ) . '"); return false;' ) ) );
								echo( Ui::Spinner( false, array( 'class' => array( 'ctlSpaceAfter', 'ctlVaMiddle' ), 'style' => array( 'display' => 'none' ) ) ) );
							}
							echo( Ui::TagClose( 'p' ) );
						}
		...
		Ui::PostBoxes_MetaboxAdd( 'operate', esc_html_x( 'Title', 'admin.Manage_Operate', 'seraphinite-accelerator' ) . Ui::Tag( 'span', Ui::AdminHelpBtn( Plugin::RmtCfgFld_GetLoc( $rmtCfg, 'Help.Manage_Operate' ), Ui::AdminHelpBtnModeBlockHeader ) ), true,
			function( $callbacks_args, $box )
			{
				...
				echo( Ui::TagOpen( 'div', array( 'class' => 'blck' ) ) );
				{
					...
					echo( Ui::Tag( 'div',
						Ui::Button( Wp::safe_html_x( 'Delete', 'admin.Manage_Operate', 'seraphinite-accelerator' ), true, null, null, 'button', array( 'class' => array( 'ctlSpaceAfter', 'ctlSpaceVBefore', 'ctlVaMiddle' ), 'style' => array( 'min-width' => '7em' ), 'onclick' => 'seraph_accel.Manager._int.OnCacheOp(this,2,"' . wp_create_nonce( 'op-2' ) . '");return false;' ) ) .
						Ui::Button( Wp::safe_html_x( 'Revalidate', 'admin.Manage_Operate', 'seraphinite-accelerator' ), false, null, null, 'button', array( 'class' => array( 'ctlSpaceAfter', 'ctlSpaceVBefore', 'ctlVaMiddle' ), 'style' => array( 'min-width' => '7em' ), 'onclick' => 'seraph_accel.Manager._int.OnCacheOp(this,0,"' . wp_create_nonce( 'op-0' ) . '");return false;' ) ) .
...
function GetHostingBannerContent()
{
	...
	$urlLogoImg = add_query_arg( array( 'v' => '2.29.11' ), Plugin::FileUri( 'Images/hosting-icon-banner.svg', __FILE__ ) );
...
function GetGeosList( $sett )
{
	...
	{
		foreach( $aGeos as &$geo )
			$geo[ 'parent' ] = '_';
		unset( $geo );
		$aGeos[ '_' ] = array( 'name' => esc_html_x( 'AllGeoLocations', 'admin.Manage_Operate', 'seraphinite-accelerator' ) );
	}
	...
	if( $nMaxItems )
	{
		...
		foreach( ( $siteId ? array( $siteId ) : GetSiteIds() ) as $siteIdEnum )
		{
			...
			foreach( $items as $itemState => &$itemsPerState )
			{
				...
				foreach( $a -> slice( 0, $nMaxItems ) as $id => $item )
				{
					$item[ 's' ] = $siteIdEnum;
					$itemsPerState[ $id ] = $item;
				}
				unset( $item );
		...
		foreach( $items as $itemState => $itemsPerState )
		{
			foreach( $itemsPerState as $id => $item )
			{
				...
				case 1:
					...
					switch( $stage )
					{
					case 'get':				$state = esc_html_x( 'StateInProgress_Get', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'parse':			$state = esc_html_x( 'StateInProgress_Parse', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'optDelay':		$state = esc_html_x( 'StateInProgress_OptDelay', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'contParts':		$state = esc_html_x( 'StateInProgress_ContParts', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'images':			$state = esc_html_x( 'StateInProgress_Images', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'frames':			$state = esc_html_x( 'StateInProgress_Frames', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'styles':			$state = esc_html_x( 'StateInProgress_Styles', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'scripts':			$state = esc_html_x( 'StateInProgress_Scripts', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'lazyCont':		$state = esc_html_x( 'StateInProgress_LazyCont', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'final':			$state = esc_html_x( 'StateInProgress_Final', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					case 'common':			$state = esc_html_x( 'StateInProgress_Common', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					default:				$state = esc_html_x( 'StateInit', 'admin.Manage_Queue', 'seraphinite-accelerator' ); break;
					}
				...
				case 2:
					...
					if( isset( $data[ 'rpt' ] ) )
						$stateDsc = esc_html_x( 'StateWaitingForRepeat', 'admin.Manage_Queue', 'seraphinite-accelerator' );
					else
					    $canBeCanceled = false;
					break;
				...
				if( $canBeCanceled )
					$state = vsprintf( esc_html_x( 'StateCanBeCanceled_%1$s%2$s%3$s', 'admin.Manage_Queue', 'seraphinite-accelerator' ), Gen::ArrFlatten( array( $state, Ui::Link( array( '', '' ), '#', false, null, array( 'onclick' => 'seraph_accel.Manager._int.OnQueueItemCancel(this,"' . $item[ 's' ] . '","' . bin2hex( $id ) . '","' . wp_create_nonce( 'cancel' ) . '");return false;' ) ) ) ) );
				...
				$res .= Ui::Tag( 'tr',
					...
					Ui::Tag( 'td', $state . ( $stateDsc ? ( Ui::Tag( 'em', '<br>' . str_replace( array( "\n", '{TAG_U_OPEN}', '{TAG_U_CLOSE}' ), array( '<br>', '<u>', '</u>' ), htmlspecialchars( wordwrap( $stateDsc ) ) ) ) ) : '' ), array( 'class' => 'ctlNoWrap cellSpaceAfter' ) ) .
...
function OnAdminApi_QueueItemCancel( $args )
{
	...
	$siteId = Gen::SanitizeId( $args[ 'siteId' ] );
	$id = hex2bin( Gen::SanitizeId( $args[ 'id' ] ) );
	...
	if( $item = $aInitial[ $id ] )
	{
		...
		$data = Gen::GetArrField( Gen::Unserialize( ($item[ 'd' ]??null) ), array( '' ), array() );
		$data[ 'hr' ] = Gen::S_ABORTED;
		$item[ 'd' ] = Gen::Serialize( $data );

		$aDone[ $id ] = $item;
	}
	else if( $item = $aProgress[ $id ] )
	{
		$data = Gen::GetArrField( Gen::Unserialize( ($item[ 'd' ]??null) ), array( '' ), array() );

		if( $fileCtl = ProcessCtlData_GetFullPath( ($data[ 'pc' ]??null) ) )
	...
	$aDone -> dispose(); $aProgress -> dispose(); $aInitial -> dispose(); $lock -> Release();
	return( Gen::S_OK );
}
...
function OnAdminApi_CacheOpBegin( $args )
{
	...
	if( !wp_verify_nonce( ($args[ '_wpnonce' ]??''), 'op-' . $args[ 'op' ] ) )
		return( Gen::E_CONTEXT_EXPIRED );
...
function OnAdminApi_CacheOpCancel( $args )
{
	if( !current_user_can( 'manage_options' ) )
		return( Gen::E_ACCESS_DENIED );
	if( !wp_verify_nonce( ($args[ '_wpnonce' ]??''), 'op-cancel' ) )
		return( Gen::E_CONTEXT_EXPIRED );
...
function OnAdminApi_HtmlCheck( $args )
{
	...
	$contentRaw = $content = str_replace( "\t", '    ', str_replace( "\r", '', $content ) );
	...
	try
	{
		if( !@$doc -> loadHTML( $content, LIBXML_BIGLINES  | LIBXML_NONET | LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_PARSEHUGE | LIBXML_PEDANTIC ) )
			$aValidationError[] = 'failed to load';
	}
	catch( \Exception $e )
	{
		for( ; $e; $e = $e -> getPrevious() )
		{
			$aValidationError[] = array( 'severity' => 'error', 'text' => $e -> getMessage() );
	...
	$fmtSrc_Open = Ui::TagOpen( 'span', array( 'style' => array( 'background-color' => '#f0f0f0' ) ) );
	$fmtSrc_Close = Ui::TagClose( 'span' );

	$fmtWarnSize_Open = Ui::TagOpen( 'span', array( 'style' => array( 'color' => '#ffba00', 'font-weight' => 'bold' ) ) );
	$fmtWarnSize_Close = Ui::TagClose( 'span' );
	...
	foreach( libxml_get_errors() as $e )
	{
		...
		case LIBXML_ERR_ERROR:
			$severity = 'warning';
			if( preg_match( '/^ID\\s+((?:\\w|-)+)\\s+already defined/i', $e -> message, $m, PREG_OFFSET_CAPTURE ) )
			{
				$e -> message = substr_replace( $e -> message, '"' . $m[ 1 ][ 0 ] . '"', $m[ 1 ][ 1 ], strlen( $m[ 1 ][ 0 ] ) );
				$severity = 'info';
			}
		...
		if( $e -> column - 1 >= 100 )
			$text .= '...' . $fmtSrc_Open . Ui::EscHtml( mb_substr( $lineText, $e -> column - 1 - 100, 100 ), true );
		else
			$text .= $fmtSrc_Open . Ui::EscHtml( mb_substr( $lineText, 0, $e -> column - 1 ), true );
		...
		$aValidationError[] = array( 'severity' => $severity, 'text' => Ui::EscHtml( 'Line ' . $e -> line . ', pos ' . $e -> column . ': ', true ) . $text );
	}
	...
	$ctxProcess = GetContentProcessCtx( array( 'REQUEST_SCHEME' => $aUrl[ 'scheme' ], 'SERVER_NAME' =>  $aUrl[ 'host' ], 'SERVER_PORT' => 443, 'REQUEST_TIME' => '1234', 'HTTP_USER_AGENT' => 'none', 'REQUEST_URI' => Net::UrlDeParse( $aUrl, 0, array( PHP_URL_PATH, PHP_URL_QUERY ) ) ), Plugin::SettGet() );
