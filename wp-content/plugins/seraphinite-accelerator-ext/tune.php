...
function _SelfDiag_GetResponseResString( $url, $requestRes, $body = false )
{
	...
	if( $body )
	{
		...
		$res .= '<br>' . Ui::Tag( 'u', Wp::safe_html_x( 'Body', 'admin.SelfDiag', 'seraphinite-accelerator' ) ) . '<br>' . str_replace( array( "\r", "\n", "\t", ' ' ), array( '', '<br>', '&nbsp;&nbsp;&nbsp;&nbsp;', '&nbsp;' ), htmlspecialchars( $bodyC ) );
	}
...
function OnAdminApi_SelfDiag_VendorSrv( $args )
{
	...
	if( $res -> hr !== Gen::S_OK )
	{
		$res -> descr = _SelfDiag_GetResponseResString( '', $requestRes );
		return( $res );
	}

	if( ( Gen::GetArrField( $sett, array( 'cache', 'procEngn' ), 1 ) & 2 ) )
	{
		foreach( Gen::GetArrField( PluginRmtCfg::Get(), 'Prms.UrlsProcMgr', array() ) as $procUrl )
		{
			$procUrl = Gen::StrReplaceKeyed( array( '{FN}' => 'TestConn', '{EP}' => rawurlencode( PluginLic::GetEpName() ), '{V}' => rawurlencode( '2.29.11' ), '{CB}' => rawurlencode( Net::UrlAddArgs( Wp::GetSiteRootUrl( '/', false ), array( 'seraph_accel_at' => 'ORC', 'f' => 'TestConn', '_n' => Gen::GetNonce( 'ORC', GetSalt(), 300 ) ) ) ) ), $procUrl );
			$requestRes = Wp::RemotePost( $procUrl, array( 'timeout' => 30, 'sslverify' => false ) );
			$res -> hr = Net::GetHrFromWpRemoteGet( $requestRes, false, true );
			if( $res -> hr == Gen::S_OK )
				break;
		}

		if( $res -> hr !== Gen::S_OK )
			$res -> descr = _SelfDiag_GetResponseResString( '', $requestRes, true );
	}
...
function SelfDiag_DetectStateAnd3rdPartySettConflicts( $cb, $ext = false )
{
	...
	if( $isContProcEnabled )
	{
		...
		else if( !class_exists( 'seraph_accel\\DomElementEx' ) )
