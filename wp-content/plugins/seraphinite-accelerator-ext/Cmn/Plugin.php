...
class PluginOptions
{
	...
	static private $_blogId = null;

	static function Get( $ver, $name, $cbNs = '', $rawExtData = null )
	{
		$aKey = array( PluginOptions::GetMultiSitePrefix(), $name );

		$data = Gen::GetArrField( self::$_cache, $aKey );
		if( $data )
			return( $data );
		...
		Gen::SetArrField( self::$_cache, $aKey, $data );
		return( $data );
	}
...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckPackage === null && $lastCheckVer !== null )
			$lastCheckPackage = 'Full';

		if( $lastCheckVer !== '2.29.11' || $lastCheckPackage !== 'Full' )
		{
			$state = Plugin::StateGet();

			if( $lastCheckVer !== '2.29.11' && !isset( $state[ 'changeVerCheck' ] ) )
			{
				$state[ 'changeVerCheck' ] = $lastCheckVer !== null ? $lastCheckVer : '';
				Plugin::StateSet( $state );
			}

			if( $lastCheckPackage !== 'Full' && !isset( $state[ 'changePkCheck' ] ) )
			{
		...
		if( !$bForce )
		{
			if( $bFirstTimeOnly && $lastCheckVer == '2.29.11' )
				return( Gen::S_FALSE );
		...
		$requestRes = Wp::RemoteGet( $urlRemoteCfg, array( 'timeout' => $bForce ? ( is_int( $bForce ) ? $bForce : 30 ) : 5, 'redirection' => 5 ) );
	...
	static function Get( $bForce = false )
	{

		PluginRmtCfg::Update( $bForce );
		$data = PluginOptions::Get( self::STG_VER, self::STG_ID, __CLASS__ . '::' );
		return( $data[ 'data' ] );
...
class Plugin
{
	...
	static function Init()
	{
		...
		Gen::CallFunc( 'seraph_accel\\OnPreInit', array( $isAdminMode ) );
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.29.11' ), '2.29.11' ) > 0 )
					{
		...
		esc_html__( 'Seraphinite Accelerator (Full, premium)', 'seraphinite-accelerator' );
	...
	static private function _AsyncTasksProcess_Task( $taskName, $taskRunTime )
	{
		...
		if( Gen::FileContentExclusive_Open( $h, OnAsyncTasksGetFile(), 5, 'cb+' ) == Gen::S_OK )
		{
			...
			if( $dataItem )
			{
				if( !Gen::FileContentExclusive_Put( $h, $data ? @serialize( array( 'data' => $data ) ) : '' ) )
					$dataItem = null;
				else if( isset( $dataItem[ 's' ] ) && Gen::FileContentExclusive_Open( $hTaskCtl, OnAsyncTasksGetFile() . $dataItem[ 's' ], 5, 'cb+' ) != Gen::S_OK )
				{
					Gen::CallFunc( 'seraph_accel\\LogWrite', array( 'Can\'t lock \'' . OnAsyncTasksGetFile() . $dataItem[ 's' ] . '\' file within 5 seconds', Ui::MsgErr, 'AsyncTasks' ) );
					$dataItem = null;
				}
			}
			...
			Gen::CallFunc( 'seraph_accel\\LogWrite', array( 'Can\'t lock \'' . OnAsyncTasksGetFile() . '\' file within 5 seconds', Ui::MsgErr, 'AsyncTasks' ) );
	...
	static private function _AsyncTasks_GetMode( $forceReRead = false )
	{
		...
		if( $g_mode === null || $forceReRead )
		{
			$settGlob = Plugin::SettGetGlobal( null, $forceReRead );
			$g_mode = ( string )Gen::CallFunc( 'seraph_accel\\OnAsyncTasksPushGetMode', array( $settGlob ), '' );
		}
	...
	static private function _AsyncTasksProcessMgr( $asyncModeRun, $bMt = true, $cbFilter = null, $bYeld = true, $tmMaxRunWait = 30, $tmMaxRun = 60, $tmStart = null )
	{

		$tmAsyncModeCheck = time();
		if( !$tmStart )
			$tmStart = $tmAsyncModeCheck;
	...
	static private function _AsyncTaskGetNextRun( &$data, $mark, $cbFilter = null, &$changed = null )
	{
		...
		for( $i = 0; $i < count( $data ); $i++ )
		{
			...
			if( $mark )
			{
				$dataItem[ 'tr' ] = ( float )sprintf( '%.3F', $tmCur );
				$changed = true;
			}
	...
	static function AsyncTaskPostEx( $name, $fast = false , $args = null, $times = Plugin::ASYNCTASK_TTL_DEF, $push = true, $singleton = false  )
	{
		...
		$hr = Gen::FileContentExclusive_Open( $h, OnAsyncTasksGetFile(), 5, 'cb+' );
		if( !$h )
		{
			Gen::CallFunc( 'seraph_accel\\LogWrite', array( 'Can\'t lock \'' . OnAsyncTasksGetFile() . '\' file within 5 seconds', Ui::MsgErr, 'AsyncTasks' ) );
			return( $hr );
	...
	{
		Gen::FileContentExclusive_Open( $h, OnAsyncTasksGetFile(), 5, 'cb+' );
		if( !$h )
		{
			Gen::CallFunc( 'seraph_accel\\LogWrite', array( 'Can\'t lock \'' . OnAsyncTasksGetFile() . '\' file within 5 seconds', Ui::MsgErr, 'AsyncTasks' ) );
			return( false );
	...
	static function AsyncTaskPushGetTimerun( $timeRun )
	{
		return( str_replace( '.', '_', sprintf( '%.3F', $timeRun ) ) );
	}

	static function AsyncTaskPushGetUrlEx( $urlBase, $id, $timeRun = null )
	{

		if( $timeRun === null )
			$timeRun = microtime( true );

		return( Net::UrlAddArgs( $urlBase, array( 'seraph_accel_at' => $id, 'rt' => Plugin::AsyncTaskPushGetTimerun( $timeRun ) ) ) );
	}

	static function AsyncTaskPushGetUrl( $id, $timeRun = null )
	{
		return( Plugin::AsyncTaskPushGetUrlEx( Wp::GetSiteWpRootUrl( Gen::CallFunc( 'seraph_accel\\OnAsyncTasksGetPushUrlFile', array(), 'wp-cron.php' ) ), $id, $timeRun ) );
	}
	...
	static function GetPluginString( $id, $context = null )
	{
		$id = 'Plugin' . $id;
		return( esc_html_x( $id, $context ? $context : 'admin.Common', 'seraphinite-accelerator' ) );
		...
		esc_html_x( 'HumanTimeDiffDigitbefore_%1$s%2$s', 'admin.Common', 'seraphinite-accelerator' );
	...
	static function GetAboutPluginContent()
	{
		...
		$urlAboutPluginImg = file_exists( __DIR__ . '/../Images/ProductLogo.png' ) ? add_query_arg( array( 'v' => '2.29.11' ), Plugin::FileUri( '../Images/ProductLogo.png', __FILE__ ) ) : null;
		...
		$url3rdPartySoft = file_exists( __DIR__ . '/../third-party-software.html' ) ? add_query_arg( array( 'v' => '2.29.11' ), Plugin::FileUri( '../third-party-software.html', __FILE__ ) ) : null;
	...
	static function GetAboutVendorContent()
	{
		...
		$urlAboutUsLogoImg = file_exists( __DIR__ . '/../Images/VendorLogo.png' ) ? add_query_arg( array( 'v' => '2.29.11' ), Plugin::FileUri( '../Images/VendorLogo.png', __FILE__ ) ) : null;
	...
	static function SettGet( $settRawExt = null, $clearCache = false )
	{
		if( $clearCache )
	...
	static function SettGetGlobal( $settRawExt = null, $clearCache = false )
	{
		...
		$sett = Plugin::SettGet( $settRawExt, $clearCache );
	...
	static private function _PrevVer_Check()
	{
		...
		if( empty( $warningVersionInfo ) )
		{
			...
			$verTo = self::_PrevVer_GetInt( '2.29.11' );
			if( $verTo < $verFrom )
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		$plgBaseNameCur = Plugin::GetCurBaseName();

		if( $plgBaseNameCur != Plugin::BASENAME && isset( $value -> response[ Plugin::BASENAME ] ) )
			unset( $value -> response[ Plugin::BASENAME ] );
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.29.11.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
		...
		if( ( $switch || version_compare( $verLast, '2.29.11' ) > 0 ) && !isset( $value -> response[ $plgBaseNameCur ] ) )
		{
			if( !isset( $value -> response ) )
				$value -> response = array();

			if( isset( $value -> no_update[ $plgBaseNameCur ] ) )
			{
				$value -> response[ $plgBaseNameCur ] = $value -> no_update[ $plgBaseNameCur ];
				unset( $value -> no_update[ $plgBaseNameCur ] );
			}

			else
				$value -> response[ $plgBaseNameCur ] = self::_TransientUpdatePlugins_GetInfo();

		}

		if( isset( $value -> response[ $plgBaseNameCur ] ) )
		{
			$obj = &$value -> response[ $plgBaseNameCur ];
			if( !$switch && version_compare( $verLast, '2.29.11' ) > 0 )
		...
		else if( isset( $value -> no_update[ $plgBaseNameCur ] ) )
			$value -> no_update[ $plgBaseNameCur ] -> package = $dwnldUrl;
		else
			$value -> no_update[ $plgBaseNameCur ] = self::_TransientUpdatePlugins_GetInfo();
	}
	...
	static function CmnStyle( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_enqueue_style( Plugin::CmnScriptId( $id ), add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.css' ), array(), '2.29.11' );
	...
	static function CmnScripts( $ids )
	{
		...
		foreach( $ids as $id )
		{
			...
			wp_register_script( $scrHndId, add_query_arg( Plugin::GetFileUrlPackageParams(), $fileUrl . '/' . $id . '.js' ), $deps, '2.29.11' );
			if( $id == 'Gen' )
	...
	{
		$hrUpdated = PluginRmtCfg::Update( 2 );
	...
	{
		$data = new AnyObj();
		self::_TransientUpdatePlugins_Process( $data, 'full', false );
...
?>
				...
				<script>
					(function()
					{
						...
						function _ProcessAnswerAndDeactivateAndClose( answerId, answerUserData )
						{
							...
							sendDataUrl = sendDataUrl.replace( "{PluginVersion}",				encodeURI( "2.29.11" ) );
							sendDataUrl = sendDataUrl.replace( "{PluginMode}",					encodeURI( "full" ) );
							sendDataUrl = sendDataUrl.replace( "{PluginPackage}",				encodeURI( "Full" ) );
	...
	static function OnActivate()
	{
		...
		Gen::CallFunc( 'seraph_accel\\OnActivate', array( Gen::Constant( 'WP_NETWORK_ADMIN' ) ) );
	}

	static function OnDeactivate()
	{

		Gen::CallFunc( 'seraph_accel\\OnDeactivate', array( Gen::Constant( 'WP_NETWORK_ADMIN' ) ) );
	}
...
class PluginRe
{
	...
	static function MakeRequest( $sMethod, $sUrl, $aHdrs, $nWait = true, $bBody = true )
	{
		...
		if( $code === null )
		{
			if( Gen::HrFail( $hr ) )
			{
				$res = self::_MakeErrRes( sprintf( 'seraph_accel:hr:0x%08X', $hr ), Gen::LastErrDsc_Get(), $sBody );
			...
			if( $hr !== Gen::S_OK )
			{
				$res = self::_MakeErrRes( sprintf( 'seraph_accel:hr:0x%08X', $hr ), null, $sBody );
				return( $res );
			}
	...
	static function Exec( $sMethod, $sUrl, $aHdrs, $nWaitForEnd = true, $bBody = true, &$sBody = null )
	{
		...
		{
			Gen::LastErrDsc_Set( LocId::Pack( 'FuncBlocked_%1$s', 'Common', array( 'proc_get_status' ) ) );
			return( Gen::E_ACCESS_DENIED );
		}
	...
	static function _GetPhpLauncher()
	{
	...
	static function _GetPhpLauncherEx( $sExtensionDir, $sPhpOs )
	{
		...
		if( 0 ) {}
			...
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/usr/(lib(?:64)?(?:/php[\\d\\.]*)?/(?:ext|mod))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/usr/(lib(?:64)?/php/\\d+)@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/php-fpm\\d+/usr/(lib(?:64)?/php)@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/usr/local/(lib(?:64)?/php/(?:ext|\\d+))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';

		else if( preg_match( '@/usr/local/php[\\d.]+/((?:ext|mod|lib(?:64)?))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
		else if( preg_match( '@/usr/local/lsws/lsphp\\d+/(lib(?:64)?/php/(?:ext|mod|\\d+))@i', $sExtensionDir, $aM, PREG_OFFSET_CAPTURE ) )
			$sPhpLauncher = substr( $sExtensionDir, 0, $aM[ 1 ][ 1 ] ) . 'bin/';
	...
	private static function _MakeErrRes( $code, $message, $data )
	{
		if( class_exists( '\\WP_Error' ) )
			return( new \WP_Error( $code, $message, $data ) );
		return( new AnyObj( array( 'code' => $code, 'message' => $message, 'data' => $data ) ) );
	}
