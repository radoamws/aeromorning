 ...
 *
 * @package Tribe\Events\Pro\Geo_Loc
 *
 * @since 4.4.0
 */
...
class Tribe__Events__Pro__Geo_Loc { // phpcs:ignore -- legacy class name
	...
	public function __construct() {
		...
		add_action( 'tribe_settings_after_save', [ $this, 'clear_min_max_coords_cache' ] );

		add_action( 'tribe_events_after_venue_metabox', [ $this, 'setup_overwrite_geoloc' ], 10 );
		add_action( 'tribe_events_filters_create_filters', [ $this, 'setup_geoloc_filter_in_filters' ], 1 );
		...
		add_action( 'tribe_events_pre_get_posts', [ $this, 'setup_geoloc_in_query' ] );
		add_action( 'tribe_repository_events_query', [ $this, 'setup_geoloc_in_query' ] );
		add_filter( 'tribe_events_list_inside_before_loop', [ $this, 'add_event_distance' ] );

		add_filter( 'tribe_events_importer_venue_array', [ $this, 'filter_aggregator_add_overwrite_geolocation_value' ], 10, 4 );
		add_filter( 'tribe_events_importer_venue_column_names', [ $this, 'filter_aggregator_add_overwrite_geolocation_column' ] );
	 ...
	 *
	 * @param array $args The current settings array.
	 *
	 * @return array
	 */
	public function inject_settings( $args ) {
		...
		$args = Tribe__Main::array_insert_after_key(
			...
			[
				'hideLocationSearch' => [
					...
					'tooltip'         => __( 'Removes location search field from the Events Bar on all views.', 'tribe-events-calendar-pro' ),
					'default'         => false,
	...
	public function maybe_offer_generate_geopoints() {
		...
		if ( ! Tribe__Admin__Notices::instance()->showing_notice( 'failed-venue-geocode-resolution' ) ) {
			...
			add_action( 'admin_notices', [ $this, 'show_offer_to_fix_notice' ] );
		}
	}
	...
	protected function show_venue_failure_notice( \WP_Post $venue ) {
		...
		$html = sprintf(
			'<p>' .
			_x(
				'Geolocation could not be fixed for the Venue “%1$s” (ID: %2$d). Please %3$s and %4$s.',
				'tribe-events-calendar-pro'
			) . '</p>',
			$venue->post_title,
			$venue->ID,
